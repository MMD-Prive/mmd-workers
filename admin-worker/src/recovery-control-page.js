export function renderRecoveryControlPage(input = {}) {
  ...
  const pickerVersion = escapeHtml(input.picker_intelligence_version || "");
  ...
  const initialPicker = escapeHtml(input.picker || "all");
...
<div class="pill">Taxonomy ${taxonomy}<br>SLA ${slaVersion}<br>Assignment ${assignmentVersion}<br>Picker ${pickerVersion}</div>
</header>
...
<select class="select" data-picker><option value="all">ทุก Picker</option><option value="waiting_reselection">รอลูกค้าเลือกใหม่</option><option value="authority_unavailable">Authority unavailable</option><option value="no_candidates">ไม่มี Candidate ปัจจุบัน</option><option value="selected">เลือกแล้ว</option></select>
</div>
...
function renderMetrics(q){q=q||{};metrics.innerHTML='<div class="metric"><span>Open</span><strong>'+esc(q.open_count||0)+'</strong></div><div class="metric"><span>Attention</span><strong>'+esc(q.attention_count||0)+'</strong></div><div class="metric"><span>Reselect</span><strong>'+esc(q.picker_waiting_reselection_count||0)+'</strong></div><div class="metric"><span>Picker blocked</span><strong>'+esc((Number(q.picker_authority_unavailable_count||0)+Number(q.picker_no_candidates_count||0)))+'</strong></div><div class="metric"><span>Unassigned</span><strong>'+esc(q.unassigned_count||0)+'</strong></div><div class="metric"><span>Overdue</span><strong>'+esc(q.overdue_count||0)+'</strong></div>'}
...
function renderList(rows){if(!rows||!rows.length){list.className="empty";list.textContent="ไม่พบ Recovery Case ตาม filter นี้";return}list.className="list";list.innerHTML=rows.map(function(x){var s=x.sla||{},a=x.age||{},m=x.assignment||{},p=x.picker||{};var owner=m.status==="assigned"?(m.assignee_label||"Assigned"):"Unassigned";var pickerLabel=p.revision?("Picker r"+p.revision+" · "+(p.queue_state||p.status||"other")):"Picker —";return '<button class="case-row '+(x.case_ref===selected?"active":"")+'" data-ref="'+esc(x.case_ref)+'"><strong>'+esc(x.customer&&x.customer.display_name||"Canonical Client")+"</strong><span>"+esc(x.domain)+" · "+esc(x.state)+" · "+esc(x.outcome_label||x.outcome_code)+'</span><span class="queue-meta"><span class="badge '+esc(s.status||"")+'">SLA '+esc(s.status||"unknown")+'</span><span class="badge">Age '+age(a.minutes)+'</span><span class="badge">'+esc(owner)+'</span><span class="badge">'+esc(pickerLabel)+'</span></span><span class="ref">'+esc(x.case_ref)+"</span></button>"}).join("");list.querySelectorAll("[data-ref]").forEach(function(b){b.onclick=function(){openCase(b.getAttribute("data-ref"))}})}
function renderCase(x){selected=x.case_ref;currentCase=x;var ctl=x.controls||{},non=ctl.nonterminal_outcomes||[],term=ctl.terminal_outcomes||[],m=x.assignment||{},p=x.picker||{};var assignee=m.status==="assigned"?(m.assignee_label||"Assigned"):"ยังไม่มีคนรับ";detail.className="";detail.innerHTML=
...
async function refreshList(){try{var u=new URL(API,location.origin);u.searchParams.set("limit","12");u.searchParams.set("domain",domain.value||"all");u.searchParams.set("state",state.value||"open");u.searchParams.set("assignment",assignment.value||"all");u.searchParams.set("picker",picker.value||"all");var b=await getJson(u.toString());if(b){renderMetrics(b.queue);renderList(b.cases||[])}}catch(e){list.className="empty";list.textContent="โหลดรายการไม่สำเร็จ: "+(e.error||"unavailable")}}
...
async function act(action){var flash=detail.querySelector("[data-flash]"),outcome="";if(action==="set_outcome")outcome=(detail.querySelector("[data-nonterminal]").value||"");if(action==="resolve")outcome=(detail.querySelector("[data-terminal]").value||"");if((action==="set_outcome"||action==="resolve")&&!outcome){flash.className="flash bad";flash.textContent="กรุณาเลือก outcome ก่อน";return}if(action==="refresh_picker"&&!(currentCase&&currentCase.picker&&currentCase.picker.revision)){flash.className="flash bad";flash.textContent="Case นี้ยังไม่มี picker revision ที่ refresh ได้";return}flash.className="flash";flash.textContent=action==="refresh_picker"?"กำลัง refresh canonical choices…":"กำลังบันทึก…";try{var body={case_ref:selected,action:action};if(outcome)body.outcome_code=outcome;if(action==="refresh_picker")body.picker_revision=currentCase.picker.revision;var b=await getJson(API,{method:"POST",headers:{"content-type":"application/json",accept:"application/json"},body:JSON.stringify(body)});if(!b)return;renderCase(b.case);var f=detail.querySelector("[data-flash]");if(f&&action==="refresh_picker"){f.className="flash ok";f.textContent=b.replayed?"Picker revision ปัจจุบันถูก refresh ไว้แล้ว":"Canonical choices ถูก refresh แล้ว · "+(b.picker_state||"picker updated")+" · Case Ref เดิม"}refreshList()}catch(e){flash.className="flash bad";flash.textContent="ไม่สำเร็จ: "+(e.error||"transition_rejected")}}
...
picker.addEventListener("change",function(){refreshList()});
