...
export function renderModelLineLinkPageWithAvatar() {
  const body = `<!doctype html><html lang="th"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>MMD MODEL · Model Link Queue</title><style>
:root{color-scheme:dark;--bg:#090908;--line:#393329;--gold:#d8bd83;--text:#f5f0e4;--muted:#aaa296;--ok:#72c68f;--danger:#e07f75;--blue:#86b9dc;--burg:#c995a3}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 15% 0,#2c2315 0,transparent 34%),var(--bg);color:var(--text);font:15px/1.55 -apple-system,BlinkMacSystemFont,"SF Pro Text","SF Pro Display","Helvetica Neue","Noto Sans Thai",sans-serif}.wrap{width:min(1160px,100%);margin:auto;padding:24px}.top,.row,.identity{display:flex}.top{gap:16px;align-items:flex-start;justify-content:space-between;margin-bottom:24px}.eyebrow{font-size:11px;letter-spacing:.18em;color:var(--gold);font-weight:700}.top h1{font-size:clamp(25px,4vw,42px);line-height:1.08;margin:7px 0}.top p{color:var(--muted);max-width:790px;margin:0}.back{color:var(--text);text-decoration:none;border:1px solid var(--line);padding:10px 13px;border-radius:12px;white-space:nowrap}.stats{display:flex;flex-wrap:wrap;gap:10px;margin:16px 0 22px}.pill,.badge{border:1px solid var(--line);background:#14120f;border-radius:999px;padding:8px 12px;color:var(--muted)}.pill b{color:var(--text)}.queue{display:grid;gap:14px}.card{border:1px solid var(--line);background:linear-gradient(180deg,#15130f,#0f0e0c);border-radius:18px;padding:18px}.row{gap:12px;justify-content:space-between;align-items:flex-start}.identity{gap:12px;align-items:flex-start;min-width:0}.identity-copy{min-width:0}.avatar{width:60px;height:60px;border-radius:50%;overflow:hidden;position:relative;flex:0 0 auto;border:1px solid rgba(216,189,131,.38);background:#251f17;display:grid;place-items:center;color:var(--gold);font-weight:800;font-size:20px}.avatar img{width:100%;height:100%;object-fit:cover;display:block;position:relative;z-index:2}.avatar .initial{position:absolute;inset:0;display:grid;place-items:center;z-index:1}.name{font-size:20px;font-weight:700;overflow-wrap:anywhere}.verified{font-size:10px;letter-spacing:.1em;color:var(--ok);font-weight:800;margin-top:2px}.meta{font-size:12px;color:var(--muted);margin-top:4px}.status{font-size:11px;letter-spacing:.08em;border:1px solid var(--line);border-radius:999px;padding:6px 9px;color:var(--gold);white-space:nowrap}.search-wrap{margin-top:16px}.lane-tabs{display:flex;flex-wrap:wrap;gap:7px;margin-bottom:9px}.lane-btn{padding:7px 11px;border-radius:999px;background:#17140f;color:var(--muted);border:1px solid var(--line);font-weight:700;font-size:12px}.lane-btn.active{background:#e6cf9a;color:#17120b;border-color:#e6cf9a}.search{display:grid;grid-template-columns:1fr auto;gap:8px}input,button{font:inherit}input{min-width:0;background:#0b0a09;border:1px solid var(--line);border-radius:12px;color:var(--text);padding:12px 13px}button{border:0;border-radius:12px;background:var(--gold);color:#1b160e;font-weight:800;padding:12px 16px;cursor:pointer}button.secondary{background:#242019;color:var(--text);border:1px solid var(--line)}button:disabled{opacity:.45;cursor:not-allowed}.results{display:grid;gap:9px;margin-top:10px}.source-note{font-size:11px;color:var(--muted);margin:7px 2px}.candidate{border:1px solid var(--line);border-radius:14px;padding:13px;background:#0b0a09;display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center}.candidate strong{display:block;font-size:16px}.candidate-top{display:flex;flex-wrap:wrap;gap:6px;align-items:center}.mini{display:inline-flex;align-items:center;border:1px solid var(--line);border-radius:999px;padding:3px 7px;font-size:9px;letter-spacing:.08em;font-weight:800}.mini.drive{color:var(--gold)}.mini.airtable{color:var(--ok)}.mini.public{color:var(--blue)}.mini.private{color:var(--burg)}.folder{font-size:12px;color:var(--muted);word-break:break-word;margin-top:5px}.action-drive{background:#e6cf9a}.empty,.notice{border:1px dashed var(--line);border-radius:18px;padding:28px;color:var(--muted);text-align:center}.empty b{display:block;color:var(--text);font-size:16px;margin-bottom:6px}.empty p{margin:0 auto 14px;max-width:680px}.empty button{margin-top:2px}.notice{margin-bottom:14px}.notice.ok{border-style:solid;color:var(--ok)}.notice.err{border-style:solid;color:var(--danger)}@media(max-width:700px){.wrap{padding:18px 14px}.top{display:block}.back{display:inline-block;margin-top:16px}.avatar{width:52px;height:52px;font-size:18px}.row{gap:8px}.status{font-size:9px;padding:5px 7px}.search,.candidate{grid-template-columns:1fr}.candidate button{width:100%}}
</style></head><body><main class="wrap"><div class="top"><div><div class="eyebrow">MMD MODEL · OWNER REVIEW</div><h1>Model Link Queue</h1><p>คิวเดียวสำหรับ Public และ Private Model รวมถึงคนที่สมัครกับ MMD ทาง LINE แล้วมีโฟลเดอร์อยู่ใน MMD Catalogue เดิม ระบบค้นได้ทั้ง Canonical Model ใน Airtable และ Approved Model Folder ใน Drive แต่จะไม่เดาหรือเชื่อม LINE ให้อัตโนมัติ เปอร์เป็นคนยืนยัน Model ที่ถูกต้องก่อนทุกครั้ง</p></div><a class="back" href="/internal/admin/kenji">← Kenji Admin</a></div><div id="notice"></div><div class="stats"><div class="pill">Waiting <b id="count">—</b></div><div class="pill">Search default: <b>ทั้งหมด</b></div><div class="pill">Fail closed · ชื่อ/รูปใช้ช่วยค้นเท่านั้น</div></div><section id="queue" class="queue"><div class="empty"><b>กำลังโหลด Pending Model Link…</b><p>กำลังอ่านคิวจาก backend</p></div></section></main><script>
(function(){'use strict';
...
function notice(msg,type=''){noticeBox.innerHTML=msg?'<div class="notice '+type+'">'+esc(msg)+'</div>':''}
function queueLoading(){count.textContent='…';queue.innerHTML='<div class="empty"><b>กำลังโหลด Pending Model Link…</b><p>กำลังอ่านคิวจาก backend</p></div>'}
function queueError(msg){count.textContent='!';queue.innerHTML='<div class="empty"><b>โหลดคิวไม่ได้</b><p>'+esc(msg||'Queue API ไม่ตอบกลับ กรุณาลองใหม่')+'</p><button class="secondary" type="button" data-action="reload-claims">ลองโหลดอีกครั้ง</button></div>'}
...
async function load(){try{notice('');queueLoading();const d=await api('/v1/admin/models/activation-candidates?mode=line-link-claims',{timeout:QUEUE_TIMEOUT_MS});const items=d.items||[];count.textContent=d.count??items.length;render(items)}catch(e){notice('โหลดคิวไม่ได้: '+(e.message||e),'err');queueError(e.message||String(e))}}
function render(items){if(!items.length){queue.innerHTML='<div class="empty"><b>ยังไม่มี Model ที่รอเชื่อม LINE</b><p>เมื่อ Model ยืนยัน LINE แล้ว รายการจะขึ้นที่นี่ ไม่ว่าจะมาจาก Public, Private หรือรับสมัครกันทาง LINE</p><button class="secondary" type="button" data-action="reload-claims">โหลดใหม่</button></div>';return}queue.innerHTML=items.map(x=>'<article class="card" data-claim="'+esc(x.claim_id)+'" data-lane="all"><div class="row"><div class="identity">'+avatar(x)+'<div class="identity-copy"><div class="name">'+esc(x.line_display_name||'LINE identity')+'</div><div class="verified">LINE VERIFIED · MODEL CLAIM</div><div class="meta">Verified '+esc(x.verified_at||'—')+' · '+esc(x.environment||'published')+' · Ref '+esc(x.line_ref||'—')+'</div></div></div><span class="status">'+esc(x.claim_status)+'</span></div><div class="search-wrap">'+laneTabs()+'<div class="search"><input value="'+esc(x.line_display_name||'')+'" placeholder="ค้นชื่อ / code เช่น Book EI / Babe B / EMs01"><button class="secondary" type="button" data-action="search-model">ค้น Model</button></div></div><div class="results"></div></article>').join('')}
