...
<body>
  ...
  <script>(()=>{const form=document.getElementById('adminLoginForm');const input=document.getElementById('adminCredential');const toggle=document.querySelector('.mmd-login21__toggle');if(!form||!input||!toggle)return;const supportsTextSecurity=Boolean(window.CSS&&CSS.supports&&CSS.supports('-webkit-text-security','disc'));if(!supportsTextSecurity)input.type='password';form.addEventListener('submit',(event)=>{if(!input.value){event.preventDefault();input.focus();return;}input.readOnly=true;});toggle.addEventListener('click',function(){const show=this.getAttribute('aria-pressed')!=='true';if(supportsTextSecurity){input.classList.toggle('is-visible',show);}else{input.type=show?'text':'password';}this.textContent=show?'HIDE':'SHOW';this.setAttribute('aria-pressed',String(show));});})();</script>
</body>
</html>`;
  ...
  return new Response(request.method.toUpperCase() === "HEAD" ? null : html, {
    ...
    headers: {
      ...
      "x-mmd-login-ui": "direct-no-autofill-v2",
