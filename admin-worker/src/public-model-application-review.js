...
const ASSET_ID_RE = /^pma_asset_[A-Za-z0-9_-]{8,120}$/;
...
export async function handlePublicModelApplicationReviewRequest(request, env = {}, actor = null) {
  ...
  const asset = path.match(/^\/v1\/admin\/model-applications\/(pma_[A-Za-z0-9_-]{8,120})\/assets\/(pma_asset_[A-Za-z0-9_-]{8,120})$/);
  if (asset && method === "GET") {
...
async function serveAsset(env, applicationId, assetId) {
  ...
  if (clean(asset.fields?.[PUBLIC_MODEL_ASSET_FIELDS.uploadStatus]).toLowerCase() !== "verified") {
    return json({ ok: false, error: "asset_not_verified" }, 409);
  }
...
function normalizeApplication(record, assets) {
  ...
  return {
    ...
    height_cm: numberValue(fields[PUBLIC_MODEL_REVIEW_FIELDS.height] ?? payload.height ?? summary.height_cm),
    location: get(PUBLIC_MODEL_REVIEW_FIELDS.location, payload.location || summary.location),
    occupation: get(PUBLIC_MODEL_REVIEW_FIELDS.occupation, payload.occupation || summary.occupation),
    why_consider: clean(payload.why_consider || summary.why_consider),
    work_interests: arrayStrings(payload.work_types || payload.interested_work_types || summary.work_interests),
    public_model_category: selectName(fields[PUBLIC_MODEL_REVIEW_FIELDS.category]) || clean(payload.public_model_category),
    privacy_level: selectName(fields[PUBLIC_MODEL_REVIEW_FIELDS.privacyLevel]) || clean(payload.privacy_level),
    contact: {
...
function render(a){
 const photos=(a.assets||[]).filter(x=>String(x.content_type||'').startsWith('image/'));
 const contact=a.contact||{};
 app.className='';
 app.innerHTML='<section class="card hero"><div><div class="eyebrow">'+esc(a.application_id)+'</div><div class="name">'+val(a.nickname)+'</div><div class="chips">'+chip(a.status,statusClass(a))+chip(a.review_status)+chip(a.intake_status,statusClass(a))+'</div><div class="muted">ส่งเมื่อ '+val(a.submitted_at)+'</div></div><div class="grid"><div class="kv"><div class="k">Age</div><div class="v">'+val(a.age)+'</div></div><div class="kv"><div class="k">Height</div><div class="v">'+(a.height_cm?esc(a.height_cm)+' cm':'—')+'</div></div><div class="kv"><div class="k">Location</div><div class="v">'+val(a.location)+'</div></div><div class="kv"><div class="k">Occupation</div><div class="v">'+val(a.occupation)+'</div></div></div></section>'+
 ...
 '<section class="card"><div class="section-title">รูปที่ส่งมา ('+photos.length+')</div><div class="photos">'+(photos.length?photos.map(x=>'<div class="photo"><img loading="lazy" src="'+esc(x.url)+'" alt="'+esc(x.role||x.kind||'photo')+'"><span>'+esc(x.role||x.kind||'photo')+'</span></div>').join(''):'<div class="muted">ไม่มีรูปที่อ่านได้</div>')+'</div></section>'+
 '<section class="card"><div class="section-title">ข้อมูลสำหรับตัดสินใจ</div><div class="grid"><div class="kv"><div class="k">Gender</div><div class="v">'+val(a.gender)+'</div></div><div class="kv"><div class="k">Public category</div><div class="v">'+val(a.public_model_category)+'</div></div><div class="kv" style="grid-column:1/-1"><div class="k">Why consider</div><div class="v">'+val(a.why_consider)+'</div></div><div class="kv" style="grid-column:1/-1"><div class="k">Work interests</div><div class="v">'+val((a.work_interests||[]).join(', '))+'</div></div></div></section>'+
 ...
 '<section class="card"><div class="section-title">หมายเหตุการพิจารณา</div><textarea id="reviewNote" class="note" placeholder="เขียนเหตุผล / สิ่งที่ต้องติดตาม (ไม่บังคับ)"></textarea><div class="muted" style="margin-top:8px">Approve = รับใบสมัครเข้าสู่ onboarding เท่านั้น ไม่เปิด Public visibility เอง</div></section>';
 actions.hidden=false;
 actions.querySelectorAll('button').forEach(b=>b.onclick=()=>decide(b.dataset.decision,a));
}
async function decide(decision,a){
 const labels={approve:'อนุมัติใบสมัครนี้',reject:'ปฏิเสธใบสมัครนี้',screening:'ย้ายใบสมัครนี้ไปสถานะกำลังพิจารณา'};
...
async function loadQueue(){const d=await api('/v1/admin/model-applications?limit=30');app.className='';const rows=d.applications||[];app.innerHTML='<div class="queue">'+(rows.length?rows.map(a=>'<a class="card q" href="?application_id='+encodeURIComponent(a.application_id)+'"><div class="eyebrow">'+esc(a.application_id)+'</div><div class="section-title" style="margin-top:6px">'+val(a.nickname)+'</div><div class="chips">'+chip(a.status,statusClass(a))+chip(a.intake_status,statusClass(a))+'</div><div class="muted">'+val(a.age)+' ปี · '+val(a.height_cm)+' cm · '+val(a.location)+'</div></a>').join(''):'<div class="card muted">ยังไม่มีใบสมัคร Public Model</div>')+'</div>'}
