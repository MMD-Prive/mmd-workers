...
async function stableRefs(actionId) {
  ...
  return {
    booking_ref: `kenji_${digest.slice(0, 24)}`,
    session_id: `kreq_${digest.slice(0, 24)}`,
...
export function buildKenjiLv5BookingDraftPayload({ context = {}, modelAccess = {}, intent = {}, actionId = "", refs = {} } = {}) {
  ...
  const visibility = token(modelAccess?.model_access?.visibility || "public") === "private" ? "private" : "public";
  const canonicalClientId = recId(client.canonical_client_id);
  const lineUserId = lineId(client.line_user_id || context?.client_360_live?.identity?.line_user_id || intent.line_user_id);
  return compact({
...
async function writeBookingDraft(env = {}, payload = {}) {
  if (!env.SIGIL_BOOKING_WORKER?.fetch) return { ok: false, status: 503, error: "sigil_booking_binding_missing" };
  try {
    const response = await env.SIGIL_BOOKING_WORKER.fetch(new Request("https://sigil-booking-worker.internal/sigil/api/booking/intake", {
      method: "POST",
      headers: { "content-type": "application/json", "x-mmd-internal-call": "true", "x-mmd-service-binding": "admin-worker" },
      body: JSON.stringify(payload),
    }));
    const data = await response.json().catch(() => null);
    if (!response.ok || data?.ok !== true) return { ok: false, status: response.status || 502, error: clean(data?.error || "booking_draft_write_failed", 160) };
    return { ok: true, status: response.status, data };
  } catch (error) {
    return { ok: false, status: 503, error: clean(error?.message || error, 160) || "booking_draft_write_unavailable" };
  }
}
...
export async function executeKenjiLv5SupervisedAction(env = {}, input = {}) {
  ...
  const canonicalLineId = lineId(context?.client_360?.line_user_id || context?.client_360_live?.identity?.line_user_id);
  if (!canonicalLineId || canonicalLineId !== requestedLineId) {
  ...
  if (!gate.ok) {
    return { ok: false, schema: KENJI_LV5_ACTION_SCHEMA, action, status: "action_blocked", executed: false, blockers: gate.reasons, live_truth_complete: context?.live_truth_complete === true };
  }
  ...
  if (!write.ok) {
    return { ok: false, schema: KENJI_LV5_ACTION_SCHEMA, action, status: "write_failed", executed: false, error: write.error, retry_safe: true, idempotency_key: refs.idempotency_key, booking_ref: refs.booking_ref };
  }
