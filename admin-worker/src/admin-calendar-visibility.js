...
export async function inspectCalendarConnection(env = {}, fetcher = fetch) {
  ...
  const [api, bridge] = await Promise.all([
    key && eventTypeId ? readJson('https://api.cal.com/v2/event-types/' + eventTypeId, {
      authorization:'Bearer ' + key, 'cal-api-version':'2024-06-14', accept:'application/json',
    }, fetcher) : null,
    readJson('https://cal-sync-worker.malemodel-bkk.workers.dev/health', {accept:'application/json'}, fetcher),
  ]);
  ...
  return {
    ...
    outbound:{ configured:Boolean(key), api_verified:apiReady, event_type_id:eventTypeId || null,
    ...
    inbound:{ reachable:bridgeReady, mode:bridgeReady && ['shadow','active'].includes(bridge.data?.mode)?bridge.data.mode:'unknown',
...
export async function calendarPageResponse(env = {}, selectedDate = '') {
  ...
  const outboundText = out.api_verified ? 'Cal API: อ่านประเภท Internal Hold ได้แล้ว' : out.status === 'missing_cal_api_key'
    ? 'ยังไม่มี CAL_API_KEY ใน admin-worker' : out.status === 'credential_rejected'
      ? 'Cal ปฏิเสธ credential ของ admin-worker' : 'ยังยืนยันการเชื่อม Cal API ไม่ได้';
  const inboundText = !incoming.reachable ? 'ยังอ่านสถานะ cal-sync-worker ไม่ได้'
    ...
    : !incoming.mapping_ledger_configured ? 'Webhook พร้อมรับ แต่ cal-sync-worker ยังไม่มี AIRTABLE_API_KEY สำหรับบันทึก Booking UID'
    : 'Webhook และการตั้งค่าบันทึก Booking UID พร้อม · ' + incoming.mode;
  ...
  headers.set('x-mmd-calendar-surface','admin-worker-v1.1');
