...
function injectCalendarOnboardingCohortV1(html) {
  if (!clean(html) || html.includes('calendar-onboarding-cohort-v2-20260923')) return html;
  const block = `<style id="calendar-onboarding-cohort-v2-20260923">
.calv5__availability .calv5__personActions{display:none!important}.calv5__onboardingCohort{margin-top:9px}.calv5__cohortSummary{display:flex;gap:5px;flex-wrap:wrap}.calv5__cohortControl{padding:10px 10px 0}.calv5__cohortStart{width:100%;min-height:38px;border:1px solid #80642d;border-radius:10px;background:#d9bb72;color:#17130d;font-size:9px;font-weight:900;cursor:pointer}.calv5__cohortStart:disabled{opacity:.6;cursor:default}.calv5__cohortList{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:7px;padding:10px}.calv5__cohortCard{min-width:0;padding:10px;border:1px solid #2d2a30;border-radius:11px;background:#0d0c10}.calv5__cohortCard.is-next{border-color:#80642d;box-shadow:inset 0 0 0 1px rgba(217,187,114,.12)}.calv5__cohortTop{display:flex;justify-content:space-between;gap:6px;align-items:flex-start}.calv5__cohortCard strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px}.calv5__cohortBadge{padding:3px 6px;border:1px solid #4f432b;border-radius:999px;color:#d8b568;font-size:7px;font-weight:900;white-space:nowrap}.calv5__cohortBadge.is-ok{border-color:#315744;color:#79ce9e}.calv5__cohortBadge.is-wait{border-color:#3f4d5c;color:#8ab8d8}.calv5__cohortBadge.is-due{border-color:#69434a;color:#e58d98}.calv5__cohortCard p{min-height:30px;margin:6px 0 0;color:#817b74;font-size:8px;line-height:1.45}.calv5__cohortMeta{display:flex;gap:4px;flex-wrap:wrap;margin-top:7px}.calv5__cohortMeta span{padding:3px 5px;border:1px solid #29272d;border-radius:999px;color:#706a64;font-size:7px}.calv5__cohortReceipt{margin-top:7px;padding-top:7px;border-top:1px solid #211f25;color:#6e6963;font-size:7px;line-height:1.5}.calv5__cohortAction{margin-top:8px}.calv5__cohortAction button,.calv5__cohortAction a{width:100%;min-height:30px;padding:0 9px;border:1px solid #37333b;border-radius:9px;background:#151319;color:#d5c8ae;font-size:8px;font-weight:800;cursor:pointer;display:grid;place-items:center;text-decoration:none}.calv5__cohortAction button.is-primary{border-color:#80642d;background:#d9bb72;color:#17130d}.calv5__cohortAction button:disabled{opacity:.55;cursor:default}.calv5__cohortDone{display:grid;place-items:center;min-height:30px;border:1px solid #315744;border-radius:9px;color:#79ce9e;font-size:8px;font-weight:900}@media(max-width:1050px){.calv5__cohortList{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:767px){.calv5__cohortList{grid-template-columns:1fr}}
</style>
<script id="calendar-onboarding-cohort-v2-20260923-runtime">
(()=>{const boot=()=>{const root=document.querySelector('.mcal'),host=root&&root.querySelector('.calv5');if(!root||!host||host.querySelector('[data-cal-onboarding-cohort]'))return;host.querySelectorAll('.calv5__onboarding').forEach(node=>node.remove());
const esc=value=>String(value==null?'':value).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const panel=document.createElement('section');panel.className='calv5__panel calv5__onboardingCohort';panel.setAttribute('data-cal-onboarding-cohort','');panel.innerHTML='<header class="calv5__panelhead"><div><small>MMD MODEL</small><h2>Cohort 1</h2></div><div class="calv5__cohortSummary" data-cal-onboarding-summary></div></header><div class="calv5__cohortControl" data-cal-onboarding-control></div><div class="calv5__cohortList" data-cal-onboarding-list></div>';
const coverage=host.querySelector('[data-cal-coverage-health]'),availability=host.querySelector('.calv5__availability');if(coverage)coverage.insertAdjacentElement('beforebegin',panel);else if(availability)availability.insertAdjacentElement('beforebegin',panel);else host.appendChild(panel);
const summary=panel.querySelector('[data-cal-onboarding-summary]'),control=panel.querySelector('[data-cal-onboarding-control]'),list=panel.querySelector('[data-cal-onboarding-list]'),date=()=>new URL(location.href).searchParams.get('date')||'';
const priority=item=>{const b=String(item&&item.priority_bucket||'');if(b==='sla_follow_up')return'เลย SLA';if(b==='upcoming_job')return'มีงานใกล้ถึง';if(b==='line_ready')return'LINE พร้อม';if(b==='identity_repair')return'ผูกตัวตน';if(b==='commercial_active')return'งาน/ขาย active';return'Backfill'};
const reason=item=>{const b=String(item&&item.priority_bucket||'');if(b==='upcoming_job')return'มีงานภายใน 14 วัน จัด MMD MODEL ก่อนงาน';if(b==='line_ready')return'LINE เชื่อมแล้ว เหลือยืนยัน Availability ครั้งแรก';if(b==='identity_repair')return'ต้องซ่อม canonical identity ก่อนออกสิทธิ์';if(b==='commercial_active')return'มี commercial / work-history signal';if(b==='sla_follow_up')return'Reminder เดิมเลยเวลา follow-up แล้ว';return'Active operational model รอเข้า MMD MODEL'};
const outcome=item=>{const s=String(item&&item.outcome_state||'action_required');if(s==='completed')return{label:'เสร็จแล้ว',cls:'is-ok'};if(s==='waiting_for_line')return{label:'รอเชื่อม LINE',cls:'is-wait'};if(s==='waiting_for_availability')return{label:'รอยืนยัน',cls:'is-wait'};if(s==='follow_up_due')return{label:'ถึงเวลาตาม',cls:'is-due'};if(s==='blocked')return{label:'ติดตรวจ',cls:'is-due'};return{label:'ต้องทำ',cls:''}};
const when=value=>value?new Intl.DateTimeFormat('th-TH',{timeZone:'Asia/Bangkok',day:'numeric',month:'short',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(value)):'';
const reload=()=>load();
const startCohort=async button=>{const old=button.textContent;button.disabled=true;button.textContent='กำลังล็อก Cohort 1…';try{const response=await fetch('/v1/admin/calendar/availability-cohort/start',{method:'POST',credentials:'include',cache:'no-store',headers:{accept:'application/json','content-type':'application/json'},body:'{}'}),body=await response.json().catch(()=>null);if(!response.ok||!body||!body.ok)throw Error((body&&body.error)||'cohort_start_failed');button.textContent=body.already_started?'Cohort 1 เริ่มแล้ว':'ล็อก Cohort 1 แล้ว';await reload()}catch(_){button.textContent='เริ่ม Cohort ไม่สำเร็จ'}finally{setTimeout(()=>{button.disabled=false;button.textContent=old},2400)}};
const remind=async button=>{const key=button.dataset.modelKey||'',old=button.textContent;button.disabled=true;button.textContent='กำลังส่ง…';try{const response=await fetch('/v1/admin/calendar/availability-reminder',{method:'POST',credentials:'include',cache:'no-store',headers:{accept:'application/json','content-type':'application/json'},body:JSON.stringify({model_key:key})}),body=await response.json().catch(()=>null);if(response.status===429&&body&&body.error==='availability_reminder_cooldown'){button.textContent='เตือนแล้ววันนี้';await reload();return}if(response.status===409&&body&&body.error==='availability_already_fresh'){button.textContent='ยืนยันแล้ว';await reload();return}if(!response.ok||!body||!body.ok)throw Error((body&&body.error)||'reminder_failed');button.textContent='ส่งแล้ว';await reload()}catch(e){button.textContent=e&&e.message==='availability_cohort_not_started'?'เริ่ม Cohort ก่อน':'ส่งไม่สำเร็จ'}finally{setTimeout(()=>{button.disabled=false;button.textContent=old},2600)}};
const activate=async button=>{const record=button.dataset.modelRecord||'',key=button.dataset.modelKey||'',old=button.textContent;button.disabled=true;button.textContent='กำลังสร้าง…';try{const response=await fetch('/v1/admin/calendar/availability-activation',{method:'POST',credentials:'include',cache:'no-store',headers:{accept:'application/json','content-type':'application/json'},body:JSON.stringify({model_record_id:record,model_key:key})}),body=await response.json().catch(()=>null);if(response.status===409&&body&&body.error==='model_already_linked'){button.textContent='LINE เชื่อมแล้ว';await reload();return}if(!response.ok||!body||!body.ok||!body.activation_url)throw Error((body&&body.error)||'activation_failed');let copied=false;try{if(navigator.clipboard&&navigator.clipboard.writeText){await navigator.clipboard.writeText(body.activation_url);copied=true}}catch(_){}if(!copied)window.prompt('คัดลอก LINE activation link สำหรับ Model',body.activation_url);button.textContent=copied?'คัดลอก LINE link แล้ว':'สร้าง LINE link แล้ว';await reload()}catch(e){button.textContent=e&&e.message==='availability_cohort_not_started'?'เริ่ม Cohort ก่อน':'สร้าง link ไม่สำเร็จ'}finally{setTimeout(()=>{button.disabled=false;button.textContent=old},3200)}};
const action=(item,tracking)=>{if(tracking&&tracking.start_required)return'<button disabled>เริ่ม Cohort 1 ก่อน</button>';const state=String(item&&item.outcome_state||'');if(state==='completed')return'<div class="calv5__cohortDone">ยืนยันแล้ว · '+esc(item.safe_availability_state||'updated')+'</div>';if(state==='waiting_for_line')return'<button disabled>รอ Model เชื่อม LINE</button>';if(state==='waiting_for_availability')return'<button disabled>รอ Model ยืนยัน Availability</button>';const next=tracking&&tracking.next_owner_action||null;if(tracking&&tracking.action_sequence_mode==='one_at_a_time'&&next&&['action_required','follow_up_due'].includes(state)&&String(item&&item.name||'')!==String(next.name||''))return'<button disabled>รอคิวก่อนหน้า · '+esc(next.name||'Model')+'</button>';const stage=String(item&&item.recovery_stage||'');if(stage==='identity_recovery_required')return'<a href="/internal/ceo/models">ผูก Model Key</a>';if((stage==='line_link_required'||stage==='line_link_expired')&&item.record_id&&item.model_key)return'<button data-onboarding-activate data-model-record="'+esc(item.record_id)+'" data-model-key="'+esc(item.model_key)+'">'+(stage==='line_link_expired'?'ออก LINE link ใหม่':'สร้าง LINE link')+'</button>';if((stage==='availability_confirmation_required'||stage==='reminder_follow_up_due')&&item.model_key)return'<button class="is-primary" data-onboarding-remind data-model-key="'+esc(item.model_key)+'">'+(stage==='reminder_follow_up_due'?'ติดตาม LINE':'เตือนยืนยัน Availability')+'</button>';return'<button disabled>รอตรวจสถานะ</button>'};
const render=onboarding=>{if(!onboarding||onboarding.schema!=='mmd.availability.onboarding-cohorts.v1'){summary.textContent='อ่าน cohort ไม่ได้';control.innerHTML='';list.innerHTML='<div class="calv5__empty">Onboarding cohort unavailable</div>';return}const tracking=onboarding.tracking||{},batch=Array.isArray(onboarding.current_batch)?onboarding.current_batch:[],remaining=Number(onboarding.remaining_after_current||0),c=tracking.counts||{},next=tracking.next_owner_action||null;summary.innerHTML='<span class="calv5__availchip">'+(tracking.state==='not_started'?'ยังไม่เริ่ม':tracking.state==='completed'?'ครบแล้ว':'กำลังทำ')+'</span><span class="calv5__availchip">เสร็จ <b>'+Number(c.completed||0)+' / '+batch.length+'</b></span><span class="calv5__availchip">รอ <b>'+Number(c.waiting_for_line||0)+Number(c.waiting_for_availability||0)+'</b></span>'+(Number(c.follow_up_due||0)?'<span class="calv5__availchip">ตามผล <b>'+Number(c.follow_up_due||0)+'</b></span>':'')+(next?'<span class="calv5__availchip">ทำต่อ <b>'+esc(next.name||'Model')+'</b></span>':'')+(remaining?'<span class="calv5__availchip">คิวถัดไป <b>'+remaining+'</b></span>':'')+'<span class="calv5__availchip">ทีละ 1 คน</span>';control.innerHTML=tracking.start_required?'<button class="calv5__cohortStart" data-onboarding-start>เริ่ม Cohort 1 · ล็อก 5 คนนี้</button>':tracking.cohort_complete?'<div class="calv5__cohortDone">Cohort 1 ครบ '+Number(tracking.completion_percent||100)+'%</div>':next?'<div class="calv5__cohortDone">Action ถัดไป · '+esc(next.name||'Model')+'</div>':'';list.innerHTML=batch.length?batch.map(item=>{const o=outcome(item),job=item.upcoming_job_at?when(item.upcoming_job_at):'',last=item.last_owner_action_at?when(item.last_owner_action_at):'',done=item.completed_at?when(item.completed_at):'',isNext=next&&String(next.name||'')===String(item.name||'');return'<article class="calv5__cohortCard'+(isNext?' is-next':'')+'"><div class="calv5__cohortTop"><strong>'+esc(item.name||item.model_id||'Model')+'</strong><span class="calv5__cohortBadge '+o.cls+'">'+esc(o.label)+'</span></div><p>'+esc(reason(item))+'</p><div class="calv5__cohortMeta"><span>'+esc(priority(item))+'</span>'+(job?'<span>งาน '+esc(job)+'</span>':'')+'<span>'+(item.line_connected?'LINE เชื่อมแล้ว':'LINE ยังไม่เชื่อม')+'</span>'+(item.partner_job?'<span>Partner job</span>':'')+(item.duplicate_records_collapsed?'<span>รวมซ้ำ +'+Number(item.duplicate_records_collapsed)+'</span>':'')+'</div><div class="calv5__cohortReceipt">'+(last?'Owner action ล่าสุด · '+esc(last):'ยังไม่มี Owner action')+(done?'<br>Model ยืนยัน · '+esc(done):'')+'</div><div class="calv5__cohortAction">'+action(item,tracking)+'</div></article>'}).join(''):'<div class="calv5__empty">ไม่มี Model ใน Cohort</div>';const startButton=control.querySelector('[data-onboarding-start]');if(startButton)startButton.addEventListener('click',()=>startCohort(startButton));list.querySelectorAll('[data-onboarding-remind]').forEach(button=>button.addEventListener('click',()=>remind(button)));list.querySelectorAll('[data-onboarding-activate]').forEach(button=>button.addEventListener('click',()=>activate(button)))};async function load(){try{const selected=date(),url='/v1/admin/calendar'+(selected?'?date='+encodeURIComponent(selected):'');const response=await fetch(url,{credentials:'include',cache:'no-store',headers:{accept:'application/json'}}),body=await response.json();if(!response.ok||!body||!body.ok)throw Error('calendar_unavailable');render(body.availability&&body.availability.onboarding)}catch(_){render(null)}}host.querySelectorAll('[data-cal-reload],[data-cal-prev],[data-cal-next],[data-cal-today]').forEach(button=>button.addEventListener('click',()=>setTimeout(load,320)));load()};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot()})();
</script>`;
  if (html.includes('</body>')) return html.replace('</body>', block + '</body>');
  return html + block;
}
...
export async function calendarPageResponse(request, env = {}, selectedDate = '') {
  ...
  html = injectCalendarOnboardingCohortV1(html);
