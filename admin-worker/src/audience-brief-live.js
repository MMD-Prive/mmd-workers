...
const PAYMENT_FIELDS = Object.freeze({
  paymentReference: "fldOO6SY49iDw8VBZ",
...
function selectName(value) {
  if (value && typeof value === "object") return clean(value.name, 80).toLowerCase();
  return clean(value, 80).toLowerCase();
}

function audienceJson(payload, status = 200) {
  return new Response(JSON.stringify(payload), {
    status,
    headers: {
      "content-type": "application/json; charset=utf-8",
      "cache-control": "no-store, private",
      "x-mmd-audience-brief": "backend-v2",
    },
  });
}
...
async function readLatestLineAudienceSnapshot(env = {}) {
  ...
  const status = selectName(fields[LINE_SYNC_FIELDS.status]);
...
async function readKnownLineContacts(env = {}) {
  ...
  for (const record of read.records) {
    const lineUserId = clean(record?.fields?.[CLIENT_FIELDS.lineUserId], 80);
  ...
  return {
    ...
    basis: "unique_canonical_clients_with_valid_line_user_id",
  };
}
...
async function readBookingConversion30d(env = {}, nowMs = Date.now()) {
  ...
  for (const record of read.records) {
    ...
    requests += 1;
    const status = selectName(fields[BOOKING_FIELDS.requestStatus]);
...
async function readOfficialVerifiedRevenue30d(env = {}, nowMs = Date.now()) {
  ...
  for (const record of read.records) {
    ...
    const paymentAt = parsedTime(fields[PAYMENT_FIELDS.paymentDate]);
    const verificationStatus = selectName(fields[PAYMENT_FIELDS.verificationStatus]);
    if (!verifiedAt || verificationStatus !== "verified") continue;
    if (!paymentAt || paymentAt < cutoff || paymentAt > nowMs + 24 * 60 * 60 * 1000) continue;

    const paymentReference = clean(fields[PAYMENT_FIELDS.paymentReference], 200);
    const dedupeKey = paymentReference || clean(record?.id, 120);
  ...
  return {
    ...
    currency: "THB",
    window_days: AUDIENCE_WINDOW_DAYS,
    source: "airtable_payments_payment_date_official_verified",
    basis: "payment_date_30d_and_official_verified",
  };
}
...
export async function buildAudienceBriefLive(request, env, ctx, delegatedJson) {
  const owner = await readAudienceOwner(request, env);
  if (!owner) return audienceJson({ ok: false, error: "unauthorized" }, 401);

  const nowMs = Date.now();
  const delegate = typeof delegatedJson === "function"
    ? delegatedJson
    : async () => ({ ok: false, status: 0, data: null });
  ...
  const [lineResult, contactsResult, activityResult, bookingResult, revenueResult, dashboardResult, recentResult] = await Promise.allSettled([
    ...
    delegate("/v1/admin/dashboard"),
    delegate("/v1/admin/clients/recent"),
  ]);

  const line = lineResult.status === "fulfilled" ? lineResult.value : { available: false, state: "waiting", reason: "line_ofc_snapshot_read_failed" };
  const contacts = contactsResult.status === "fulfilled" ? contactsResult.value : { available: false, state: "waiting", reason: "canonical_line_contacts_read_failed" };
  const activity = activityResult.status === "fulfilled" ? activityResult.value : { available: false, state: "waiting", reason: "customer_activity_read_failed" };
  const booking = bookingResult.status === "fulfilled" ? bookingResult.value : { available: false, state: "waiting", reason: "booking_conversion_read_failed" };
  const revenue = revenueResult.status === "fulfilled" ? revenueResult.value : { available: false, state: "waiting", reason: "official_verified_revenue_read_failed" };
  const dashboard = dashboardResult.status === "fulfilled" ? dashboardResult.value : { ok: false, status: 0, data: null };
  const recent = recentResult.status === "fulfilled" ? recentResult.value : { ok: false, status: 0, data: null };
  ...
  const sourceStatus = {
    ...
    customer_data: metricSourceStatus(contacts, {
      ...
      basis: contacts.basis || "unique_canonical_clients_with_valid_line_user_id",
    }),
    engagement: metricSourceStatus(activity, { window_days: AUDIENCE_WINDOW_DAYS }),
    booking: metricSourceStatus(booking, { window_days: AUDIENCE_WINDOW_DAYS }),
    payments: metricSourceStatus(revenue, {
      window_days: AUDIENCE_WINDOW_DAYS,
      basis: revenue.basis || "payment_date_30d_and_official_verified",
    }),
  ...
  const signals = {
    ...
    revenue: revenue.available
      ? {
          ...
          duplicates_skipped: revenue.duplicates_skipped,
  ...
  return audienceJson({
    ...
    authority: "backend",
    source: "admin-worker",
    schema: "mmd.admin.audience.brief.v2",
    ...
    guardrails: {
      ...
      historical_backfill_is_current_revenue: false,
      may_broadcast: false,
