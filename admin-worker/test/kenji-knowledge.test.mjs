...
test("internal published endpoint accepts X-Internal-Token", async () => {
  const envValue = env();
  const published = await createAndReturnCard(envValue, safeCard({ title: "Published card" }));
  await call(envValue, `/v1/admin/kenji/knowledge/${published.id}/publish`, { method: "POST", headers: adminHeaders() });

  const res = await call(envValue, "/v1/internal/kenji/knowledge/published", { headers: xInternalHeaders() });
  const body = await jsonResponse(res);
  assert.equal(res.status, 200);
  assert.equal(body.cards.length, 1);
  assert.equal(body.cards[0].id, published.id);
...
test("internal published endpoint excludes internal-only cards", async () => {
  const envValue = env();
  const publicCard = await createAndReturnCard(envValue, safeCard({ title: "Public published card", audience: "member" }));
  const internalCard = await createAndReturnCard(envValue, safeCard({ title: "Internal published card", audience: "internal_only" }));
  await call(envValue, `/v1/admin/kenji/knowledge/${publicCard.id}/publish`, { method: "POST", headers: adminHeaders() });
  await call(envValue, `/v1/admin/kenji/knowledge/${internalCard.id}/publish`, { method: "POST", headers: adminHeaders() });
  ...
  assert.notEqual(body.cards[0].id, internalCard.id);
});
