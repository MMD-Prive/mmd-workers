...
const readyFixture={schema:'job_readiness_v1',details:{job_date:'2026-10-04',start_time:'16:00',end_time:'17:30',location_name:'Previous Hotel'},missing_fields:[],checks:[{key:'details',status:'complete'},{key:'changes',status:'pending'},{key:'customer',status:'complete'},{key:'model',status:'complete'}],checklist_complete:false,next_action:'review_changes',change_requests:{source_status:'complete',open_count:1,items:[{request_id:'r1',request_type:'location_change',status:'pending_review',requested_at:'2026-09-22T10:00:00Z',current_value:{location_name:'Previous Hotel'},requested_value:{location_name:'New Hotel <img src=x onerror=alert(1)>'},remark:'Need a different place'}]},lifecycle:{state:'en_route',updated_at:'2026-10-04T08:00:00Z'}};

test('both acknowledgements with a pending change show next action, before/after and actual model status',async()=>{
 const paid={...job,payments:[{...job.payments[0],state:'paid',proof_ids:[]}]};const h=setup({items:[],jobs:[paid],fetch:async url=>url.includes('view=confirmation')?{ok:true,status:200,json:async()=>({...confirmation,confirmation:{...confirmation.confirmation,model_acknowledged_at:'2026-09-22T01:00:00Z'},readiness:readyFixture})}:null});try{await tick();await tick();h.search('Que');await h.open();const box=h.q('[data-pf-readiness]');assert.match(box.textContent,/ขั้นต่อไป: ตรวจคำขอจากลูกค้า/);assert.match(box.textContent,/Previous Hotel/);assert.match(box.textContent,/New Hotel/);assert.match(box.textContent,/กำลังเดินทาง/);assert.equal(box.querySelector('img'),null,'customer input must stay text');assert.doesNotMatch(h.q('[data-pf-confirm]').textContent,/ทั้งสองฝ่ายกดยืนยันแล้ว/);assert.equal(h.calls.filter(x=>x.opts.method==='POST').length,0);assert.equal(h.q('[data-pf-approve]'),null);
 }finally{h.dom.window.close()}
});
