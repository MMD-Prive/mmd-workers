...
const LOCK = "sigil-booking-worker-v24-airtable-resolver";
...
async function handleModelSearch(req, env, url) {
  ...
  if (scope === "private" && !privateAllowed) {
    return {
      ...
      matched: false,
      blocked: true,
      reason: "private_requires_active_member",
      access_scope: storedAccess.access_scope || "public_only",
  ...
  const allowed = records
    .map((record) => sanitizeModelForBooking(record, { scope, privateAllowed, env }))
    .filter(Boolean);
...
async function handleBookingIntake(req, env) {
  ...
  const resolverPayload = body.resolver_payload_json && typeof body.resolver_payload_json === "object"
    ? body.resolver_payload_json
  ...
  return {
    ...
    booking_ref: bookingRef,
    session_id: sessionId,
    next_url: buildNextUrl(env, body, { bookingRef, sessionId })
  };
}
...
async function resolveMemberAccess(env, input) {
  ...
  if (!member) {
    return accessOut("not_found", "", "public_only", false, "signup_or_continue_public", "not_found");
  }
  ...
  if (ledger.active || statusToken === "active" && expiryOk) {
    const tier = ledger.tier || tierFromText(f["Membership Tier"] || f["Package / Tier"]);
    return accessOut("active", tier, "public_private", true, "continue_booking", "matched");
  }

  if (statusToken === "expired" || statusToken === "inactive" || expiry && !expiryOk) {
    return accessOut("expired", tierFromText(f["Membership Tier"]), "public_only", false, "renew_for_private", "matched");
  }
...
async function findActiveMemberPackage(env, email) {
  ...
  const formula = `LOWER({member_email})=${formulaText(email.toLowerCase())}`;
  const rows = await airtableListByFormula(env, table, formula, 25);
  let best = null;
...
function sanitizeModelForBooking(record, { scope, privateAllowed, env }) {
  ...
  if (scope === "private") {
    ...
    if (!canPublic || canPrivate && !canPublic) return null;
  }
...
function buildNextUrl(env, body, ids) {
  ...
  const pay = str(env.PAY_URL || "/sigil/pay");
