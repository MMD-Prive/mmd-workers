...
async function handleReconcile(request, env) {
  ...
  if (body?.mode === "inspect") {
    ...
    for (const room of KNOWN_ROOMS) {
      ...
      if (!chatId) {
        observations.push({ room, configured: false, active: false, reason: room === "vip" ? "vip_room_unresolved" : "room_not_configured" });
        continue;
      }
      const response = await telegram(env, "getChatMember", { chat_id: chatId, user_id: userId });
      const status = String(response?.result?.status || "");
      const active = response.ok === true && !["left", "kicked"].includes(status);
      if (active) telegramRooms.push(room);
      observations.push({ room, configured: true, active, status: status || "unknown" });
    }
    return json({ ok: true, authority: "downstream_observation_only", telegram_rooms: telegramRooms, observations });
  }
  ...
  const results = [];
  for (const room of rooms) results.push(await reconcileRoom(env, room, userId, actionFor(actions, room)));
...
async function reconcileRoom(env, room, userId, action) {
  ...
  if (action === "grant") {
    ...
    if (unban.ok !== true && !String(unban.description || "").toLowerCase().includes("user not found")) return { ok: false, room, action, reason: "unban_failed" };
    const invite = await telegram(env, "createChatInviteLink", { chat_id: chatId, member_limit: 1, expire_date: Math.floor(Date.now() / 1000) + 3600, name: `mmd-reconcile-${room}`.slice(0, 32) });
