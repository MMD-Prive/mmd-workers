...
export async function handleDriveReconcile(request, env = {}) {
  ...
  if (body?.mode === "inspect") {
    ...
    for (const layer of ["standard", "premium"]) {
      ...
      const permissions = await listPermissions(token, folderId).catch(() => []);
      const active = permissions.some((p) => p.deleted !== true && normalizeEmail(p.emailAddress) === email && String(p.type || "").toLowerCase() === "user" && ACTIVE_ROLES.has(String(p.role || "")));
      if (active) driveLayers.push(layer);
    }
    return json({ ok: true, authority: "downstream_observation_only", drive_layers: driveLayers });
  }
  ...
  for (const layer of ["standard", "premium"]) {
    ...
    results.push({ layer, ...(await reconcileFolder(token, folderId, email, desired)) });
  }
...
async function reconcileFolder(token, folderId, email, action) {
  if (action === "none") return { ok: true, action };
...
function primarySource(env) {
  const source = { client_id: String(env.GOOGLE_DRIVE_CLIENT_ID || "").trim(), client_secret: String(env.GOOGLE_DRIVE_CLIENT_SECRET || "").trim(), refresh_token: String(env.GOOGLE_DRIVE_REFRESH_TOKEN || "").trim(), standard_folder_id: String(env.DRIVE_STANDARD_PACKAGE_FOLDER_ID || "").trim(), premium_folder_id: String(env.DRIVE_PREMIUM_PACKAGE_FOLDER_ID || "").trim() };
  return source.client_id && source.client_secret && source.refresh_token ? source : null;
}
async function accessToken(source) {
  const response = await fetch("https://oauth2.googleapis.com/token", { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ client_id: source.client_id, client_secret: source.client_secret, refresh_token: source.refresh_token, grant_type: "refresh_token" }) });
  const payload = await response.json().catch(() => ({}));
