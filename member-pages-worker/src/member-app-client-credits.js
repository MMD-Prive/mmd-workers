...
async function resolveCanonicalClient(env, lineUserId) {
  const records = await listAirtable(env, CLIENTS_TABLE, {
    pageSize: 3,
    filterByFormula: `{line_user_id}=${formulaString(lineUserId)}`,
  });
  if (records.length !== 1) {
    return { state: records.length === 0 ? "unresolved" : "ambiguous", clientId: null };
  }
  const clientId = clean(records[0]?.id, 40);
  return /^rec[A-Za-z0-9]{14}$/.test(clientId)
    ? { state: "resolved", clientId }
    : { state: "unresolved", clientId: null };
}

function safeCredit(record) {
  const fields = record?.fields && typeof record.fields === "object" ? record.fields : {};
  const status = normalized(fields.Status) || "unknown";
  const available = Math.max(0, asNumber(fields["Available Amount THB"]) || 0);
  const original = Math.max(0, asNumber(fields["Original Amount THB"]) || 0);
  const applied = Math.max(0, asNumber(fields["Applied Amount THB"]) || 0);
  return {
    creditId: clean(fields.credit_id, 120) || null,
    status,
    reason: normalized(fields.Reason) || "other",
    originalAmountThb: original,
    availableAmountThb: available,
    appliedAmountThb: applied,
...
async function readCredits(env, clientId) {
  const records = await listAirtable(env, CLIENT_CREDITS_TABLE, {
    pageSize: 100,
    filterByFormula: `{client_record_id}=${formulaString(clientId)}`,
  });
...
export async function handleMemberClientCredits(request, env = {}) {
  if (request.method !== "GET") {
    return response({ ok: false, error: { code: "METHOD_NOT_ALLOWED", message: "GET required." } }, 405);
  }

  const session = await readVerifiedSession(request, env);
  if (!session) {
    return response({ ok: false, error: { code: "MEMBER_SESSION_REQUIRED", message: "Verified member session required." } }, 401);
  }
  ...
  try {
    ...
    if (client.state !== "resolved") {
      return response({
        ...
        balance: null,
        items: [],
        error: { code: client.state === "ambiguous" ? "CLIENT_IDENTITY_AMBIGUOUS" : "CLIENT_IDENTITY_UNRESOLVED" },
      }, 503);
    }
    ...
    return response({
      ...
      balance: {
        currency: "THB",
        available: credits.availableBalanceThb,
      },
      items: credits.items,
    });
  } catch {
    return response({
      ...
      balance: null,
      items: [],
      error: { code: "CLIENT_CREDIT_READ_UNAVAILABLE" },
    }, 503);
  }
}
