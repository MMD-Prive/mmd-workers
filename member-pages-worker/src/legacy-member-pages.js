...
function renderMemberDashboard(request) {
  const html = `<style>#mmd-member-dashboard{padding:clamp(22px,5vw,44px);border:1px solid rgba(243,212,125,.24);border-radius:28px;background:radial-gradient(circle at 85% 0,rgba(120,71,15,.28),transparent 38%),linear-gradient(145deg,rgba(19,14,10,.96),rgba(7,5,4,.94));box-shadow:0 24px 80px rgba(0,0,0,.34)}#mmd-member-dashboard .status-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-top:24px}#mmd-member-dashboard .status-card{padding:18px;border:1px solid rgba(243,212,125,.2);border-radius:20px;background:rgba(255,255,255,.045)}#mmd-member-dashboard .status-label{color:#bcb3a2;font-size:12px}#mmd-member-dashboard .status-value{margin-top:7px;color:#fff8e9;font-size:clamp(24px,5vw,42px);font-weight:850}#mmd-member-dashboard .status-badge{display:inline-flex;margin-top:7px;padding:8px 12px;border-radius:999px;color:#120d05;background:#f3d889;font-weight:850}#mmd-member-dashboard .status-history{margin:10px 0 0;padding-left:18px;color:#ddd3c1;line-height:1.8}#mmd-member-dashboard .status-error{display:none;color:#ffcfb0}@media(max-width:680px){#mmd-member-dashboard .status-grid{grid-template-columns:1fr 1fr}#mmd-member-dashboard .status-card:last-child{grid-column:1/-1}}</style><section id="mmd-member-dashboard" aria-labelledby="mmd-member-dashboard-title"><p class="eyebrow">MMD PRIVÉ · MEMBER STATUS</p><h1 id="mmd-member-dashboard-title">My MMD</h1><p id="mmd-member-hello" class="lead">กำลังตรวจสอบสถานะสมาชิกจากระบบ MMD…</p><p id="mmd-member-error" class="status-error"></p><div class="status-grid" aria-live="polite"><article class="status-card"><div class="status-label">Points</div><div id="mmd-member-points" class="status-value">—</div><p>คะแนนที่ระบบยืนยันแล้ว</p></article><article class="status-card"><div class="status-label">Membership</div><div id="mmd-member-tier" class="status-badge">กำลังตรวจสอบ</div><p id="mmd-member-status">สถานะกำลังตรวจสอบ</p></article><article class="status-card"><div class="status-label">Recent activity</div><ul id="mmd-member-history" class="status-history"><li>กำลังโหลดข้อมูล</li></ul></article></div></section><script src="https://static.line-scdn.net/liff/edge/2/sdk.js"></script><script>(async function(){var set=function(id,text){var e=document.getElementById(id);if(e)e.textContent=text};var showError=function(text){set("mmd-member-error",text);var e=document.getElementById("mmd-member-error");if(e)e.style.display="block"};try{await liff.init({liffId:"2010298002-mbx9kqQn"});if(!liff.isLoggedIn()){if(!liff.isInClient()){showError("กรุณาเปิดหน้านี้จาก LINE Mini App ของ MMD เพื่อยืนยันตัวตนครับ");return}liff.login();return}var token=liff.getIDToken();if(!token)throw new Error("LINE identity token unavailable");var start=await fetch("/member/api/liff/start",{method:"POST",credentials:"include",headers:{"content-type":"application/json"},body:JSON.stringify({id_token:token,intent:"status",liff_intent:"status"})});if(!start.ok)throw new Error("เริ่มตรวจสอบสมาชิกไม่สำเร็จ");var profile=await fetch("/member/api/liff/profile",{credentials:"include"});var payload=await profile.json();if(!profile.ok||!payload.ok)throw new Error(payload&&payload.error&&payload.error.message||"ยังไม่พบข้อมูลสมาชิกที่ยืนยันแล้ว");var data=payload.data||{};var p=data.profile||{};set("mmd-member-hello",(p.display_name||"สมาชิก MMD")+" · ข้อมูลล่าสุดจากระบบที่ยืนยันแล้ว");set("mmd-member-points",String(p.points||0)+" pts");set("mmd-member-tier",p.tier||"Member");var status=p.membership_status||"under_review";set("mmd-member-status",status==="active"?"สถานะใช้งานอยู่":status==="grace"?"อยู่ในช่วงผ่อนผัน":status==="expired"?"สมาชิกหมดอายุ":"อยู่ระหว่างตรวจสอบ");var list=document.getElementById("mmd-member-history");if(list){list.innerHTML="";(p.history||[]).slice(0,5).forEach(function(item){var li=document.createElement("li");li.textContent=item.date+" · "+item.title+(item.type==="points"&&item.points_delta!=null?" · "+(item.points_delta>0?"+":"")+item.points_delta+" pts":"");list.appendChild(li)});if(!list.children.length){var li=document.createElement("li");li.textContent="ยังไม่มีรายการล่าสุด";list.appendChild(li)}}}catch(error){showError(error&&error.message||"ระบบตรวจสอบสมาชิกยังไม่พร้อม กรุณาลองใหม่อีกครั้ง");set("mmd-member-hello","ยังไม่สามารถยืนยันสถานะได้");}})();</script>`;
  return page(request, "member-dashboard", html);
}
