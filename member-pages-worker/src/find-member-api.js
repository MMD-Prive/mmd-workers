...
async function handleCreateRequest(request, env) {
  ...
  if (!auth.session.member_exists || !safeIdentifier(auth.session.member_id, 160)) {
    return commitError(env, auth, "FIND_MEMBER_REQUIRED", "Verified MMD membership is required.", 403);
  }

  const memberId = safeIdentifier(auth.session.member_id, 160);
  ...
  return json({
    ...
    status: "draft",
    next_action: "review",
    access_scope: safeAccessScope(resolved.payload.access_scope),
  }, 201, { cookies });
}
...
function safeModelSummary(item) {
  ...
  const modelKey = safeIdentifier(item.model_key || item.unique_key, 80);
...
function safeIdentifier(value, maxLength) {
  const text = String(value || "").trim();
  return text && text.length <= maxLength && /^[A-Za-z0-9][A-Za-z0-9._~-]*$/.test(text) ? text : "";
}
...
function safeAccessScope(value) { return ["public_only", "public_private", "private_review", "blocked"].includes(String(value || "")) ? String(value) : "public_only"; }
...
function normalizePath(pathname) { const path = String(pathname || "/").toLowerCase().replace(/\/{2,}/g, "/"); return path; }
function cookieValue(request, name) { for (const part of (request.headers.get("cookie") || "").split(";")) { const [key, ...rest] = part.trim().split("="); if (key === name) return safeIdentifier(rest.join("="), 8192); } return ""; }
