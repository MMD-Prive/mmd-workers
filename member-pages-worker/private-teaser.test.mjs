...
function fixture() {
  ...
  const env = {
    ...
    AIRTABLE_TABLE_CLIENTS:"Clients", AIRTABLE_TABLE_MEMBER_ENTITLEMENTS:"Entitlements", AIRTABLE_TABLE_MODEL_OFFER_RULES:"Rules", AIRTABLE_TABLE_MODEL_MEDIA_ASSETS:"Media", AIRTABLE_TABLE_PRIVATE_FLASH_PREVIEW_GRANTS:"Grants", AIRTABLE_TABLE_MODELS:"Models",
    ...
    AIRTABLE_HTTP:{ fetch:async (request) => {
      ...
      if (table === "Models") return Response.json({ records:[{ id:modelId, fields:{ fldShiT60bmCxFxRu:"Example Model" } }] });
...
test("public model slug resolves server-side and never returns a registry identifier", async () => {
  const f = fixture();
  const response = await handlePrivateTeaser(new Request("https://www.mmdbkk.com/api/member/app/private-teaser/availability?model_slug=example-model&work_lane=companion", {
