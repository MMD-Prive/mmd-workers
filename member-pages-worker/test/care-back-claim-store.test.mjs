...
test("CARE BACK verification creates a current-member claim, holds the coupon for a Birthday Wish, and creates one idempotent membership-benefit application", async () => {
  const writes = [];
  ...
  assert.equal(writes[1].fields.benefit_type, "discount_percent");
  assert.equal(writes[1].fields.benefit_value, 10);
  assert.equal("expires_at" in writes[1].fields, false);
  assert.match(writes[1].fields.payload_json, /"wish_submitted":false/);
  assert.equal(writes[2].table, "MMD — Campaign Benefit Applications");
  assert.equal(writes[2].fields.benefit_type, "membership_extension");
  assert.equal(writes[2].fields.status, "pending");
  assert.match(writes[2].fields.idempotency_key, /^6-years-care-back:CB6-2026-[A-F0-9]+:membership_extension$/);
