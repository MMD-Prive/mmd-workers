...
test("recognizes only TMIB ACT 001 protected routes", () => {
...
test("episode purchase requires exactly verified paid ACT 001 at 299 THB", () => {
  const good = { fields: {
    "Payment Status": "Paid",
  ...
  assert.equal(paymentGrantsTmib(good), true);
  assert.equal(paymentGrantsTmib({ fields: { ...good.fields, amount_thb: 298 } }), false);
  assert.equal(paymentGrantsTmib({ fields: { ...good.fields, "Verification Status": "Pending" } }), false);
  assert.equal(paymentGrantsTmib({ fields: { ...good.fields, package_code: "other" } }), false);
});
