...
async function page(origin){
  ...
  for(let attempt=0;attempt<12;attempt++){
    ...
    if(r.ok&&r.headers.get('x-mmd-calendar-surface')==='admin-worker-v1.2'){
...
if(!before.connection.outbound.configured){
  ...
  if(key){
    const r=await get('https://api.cal.com/v2/event-types/7057823',{authorization:'Bearer '+key,'cal-api-version':'2026-02-25'});const body=await r.json();
...
for(const origin of origins){
  ...
  results.push({origin,surface:'admin-worker-v1.2',days,connection:view.connection});
}
