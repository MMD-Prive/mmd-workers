...
export default {
  ...
  async scheduled(controller, env, ctx) {
    ...
    const sourceRef = `lifecycle:${new Date(scheduledAt).toISOString()}`;
    const job = runLifecycleReconciliation(env, {
      now: scheduledAt,
      reconcileMember: (identity) => reconcileLifecycleMember(env, identity, { source_ref: sourceRef, actor: "lifecycle_cron" }),
    }).then((summary) => {
      console.log({
...
async function reconcileLifecycleMember(env, identity, context = {}) {
  ...
  const response = await handleAccessReconcile(new Request("https://mmd-auth-worker.internal/__internal/member-access/reconcile", {
    ...
    body: JSON.stringify({ ...identity, source_ref: context.source_ref, actor: context.actor }),
  }), env);
  const payload = await response.clone().json().catch(() => null);
  return {
...
async function handleAccessReconcile(request, env) {
  ...
  const telegramUserId = canonicalTelegramUserId(body?.telegram_user_id);
  const actor = body?.actor === "lifecycle_cron" ? "lifecycle_cron" : "trusted_internal_request";
  ...
  if (snapshot.source_status !== "verified") {
    return auditedResult(env, {
      ok: false,
      error: "entitlement_snapshot_unavailable",
      entitlement_snapshot: snapshot,
    }, 503, { identity, telegram_user_id: telegramUserId, snapshot, actor, source_ref: sourceRef, reason: "entitlement_snapshot_unavailable" });
  }
  ...
  if (identity.email) {
    ...
    else return auditedResult(env, { ok: false, error: "drive_observation_failed", observations }, 503, { identity, telegram_user_id: telegramUserId, snapshot, observations, actor, source_ref: sourceRef, reason: "drive_observation_failed" });
  }
  ...
  if (telegramUserId) {
    ...
    else return auditedResult(env, { ok: false, error: "telegram_observation_failed", observations }, 503, { identity, telegram_user_id: telegramUserId, snapshot, observations, actor, source_ref: sourceRef, reason: "telegram_observation_failed" });
  }
  ...
  if (telegramUserId) applied.telegram = await downstream(env.TELEGRAM_ACCESS_RECONCILER, "https://telegram-worker.internal/telegram/internal/access/reconcile", env.AUTH_SERVICE_AUTH_TO_TELEGRAM, { telegram_user_id: telegramUserId, actions: plan.telegram });

  const appliedItems = Object.values(applied);
  if (!appliedItems.length) {
    const payload = { ok: false, error: "no_actionable_downstream_identity", entitlement_snapshot: snapshot, reconciliation_plan: plan, observations, applied };
    return auditedResult(env, payload, 409, { identity, telegram_user_id: telegramUserId, snapshot, observations, plan, applied, actor, source_ref: sourceRef, reason: "no_actionable_downstream_identity" });
  }
  const ok = appliedItems.every((item) => item?.ok === true);
  const payload = { ok, authority: "my_mmd_entitlement_resolver_v1", entitlement_snapshot: snapshot, reconciliation_plan: plan, observations, applied };
  return auditedResult(env, payload, ok ? 200 : 409, { identity, telegram_user_id: telegramUserId, snapshot, observations, plan, applied, actor, source_ref: sourceRef, reason: plan.reason, error_code: ok ? "" : firstDownstreamError(applied) });
}

async function auditedResult(env, payload, status, context = {}) {
  try {
    const audit = await writeReconciliationAudit(env, {
      member_email: context?.identity?.email,
      line_user_id: context?.identity?.line_user_id,
      ...
      error_code: context.error_code || payload?.error,
      ok: payload?.ok === true,
    });
    return json({ ...payload, audit_event_id: audit.event_id }, status);
  } catch (error) {
    const failureClass = safeFailure(error);
    console.error({ event: "my_mmd_reconciliation_audit_write_failed", failure_class: failureClass, original_status: status });
    return json({
      ok: false,
      error: "reconciliation_audit_write_failed",
      failure_class: failureClass,
      original_error: safeFailure(payload?.error || (payload?.ok === false ? "reconcile_failed" : "reconcile_applied")),
...
function firstDownstreamError(applied = {}) { for (const item of Object.values(applied)) if (item?.ok !== true) return item?.error || item?.payload?.error || "downstream_apply_failed"; return ""; }
