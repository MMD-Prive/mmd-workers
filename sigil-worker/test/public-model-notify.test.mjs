...
test("notification retry policy avoids sent/pending duplicates and retries failed or unobserved duplicates", () => {
...
test("Public Model alert is privacy-light and omits direct contact and free-text motivation", () => {
  const text = buildPublicModelTelegramMessage({
    nickname: "Tester",
    age: 28,
    height: 181,
    location: "Bangkok",
    occupation_detail: "Barista",
    work_types: ["Public Events"],
    phone: "0999999999",
    line_id: "secret-line",
    email: "private@example.com",
    why_consider: "private motivation",
    uploads: [{ kind: "photo" }, { kind: "document" }],
  }, APPLICATION_ID);
...
test("successful notification writes pending then sent audit state after persisted application lookup", async () => {
  ...
  const env = {
    ...
    AIRTABLE_FETCH: async (url, init = {}) => {
      ...
      if (method === "GET") {
        assert.match(String(url), /filterByFormula/);
        return jsonResponse({ records: [{ id: RECORD_ID, fields: {} }] });
      }
      if (method === "PATCH") {
        const body = JSON.parse(init.body);
        airtableWrites.push(body.fields);
        return jsonResponse({ id: RECORD_ID, fields: body.fields });
      }
      throw new Error(`unexpected Airtable method ${method}`);
    },
    TELEGRAM_FETCH: async (url, init = {}) => {
      telegramCalls.push({ url: String(url), body: JSON.parse(init.body) });
      return jsonResponse({ ok: true, result: { message_id: 1 } });
    },
  };

  const result = await notifyPublicModelApplication({
    env,
    payload: { nickname: "Tester", age: 28, location: "Bangkok" },
  ...
  assert.equal(airtableWrites.length, 2);
  assert.equal(airtableWrites[0][TELEGRAM_NOTIFY_FIELDS.status], "pending");
  assert.equal(airtableWrites[1][TELEGRAM_NOTIFY_FIELDS.status], "sent");
  assert.ok(airtableWrites[1][TELEGRAM_NOTIFY_FIELDS.notifiedAt]);
});

test("Telegram failure is audited as failed without mutating persisted application success", async () => {
  const airtableWrites = [];
  const env = {
    AIRTABLE_API_TOKEN: "test-airtable-token",
    TELEGRAM_CHAT_ID: "-100123",
  ...
  await assert.rejects(
    notifyPublicModelApplication({ env, payload: { nickname: "Tester" }, applicationId: APPLICATION_ID }),
    /missing_telegram_configuration/,
  );

  assert.equal(airtableWrites[0][TELEGRAM_NOTIFY_FIELDS.status], "pending");
  assert.equal(airtableWrites[1][TELEGRAM_NOTIFY_FIELDS.status], "failed");
  assert.match(airtableWrites[1][TELEGRAM_NOTIFY_FIELDS.error], /missing_telegram_configuration/);
});
