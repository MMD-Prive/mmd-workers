...
export async function executeKenjiLv5LineBookingAction({ env = {}, event = {}, modelGate = {}, decision = {}, canonicalClientId = "" } = {}) {
  ...
  const payload = {
    ...
    intent: {
      ...
      duration_source: text(parsed.duration_source, 80),
      location: text(parsed.location, 160),
      amount_thb: Number(parsed.amount_thb || 0),
      original_amount_thb: Number(parsed.original_amount_thb || 0),
      pricing_adjustment: text(parsed.pricing_adjustment, 40),
