...
| Admin APIs | `/v1/admin/*` | route-level exceptions: `POST /v1/admin/access/grant`, `/v1/admin/create-session`, `/v1/admin/create-job`, `/v1/admin/jobs/create-session`, `/v1/admin/line/liff-renewal-queue` | API hosts | GET, POST | `admin-worker` default; explicit exceptions keep current owners | direct, proxy, or exception-specific route | none | `admin-worker` default; `auth-worker` owns grant; `immigrate-worker` owns legacy create-session/create-job ingress bridges to `/v1/admin/job/create`; member-pages LIFF renewal queue remains exception pending source confirmation | LIVE CONNECTED | bridge routes source/config verified; namespace not fully enumerated here | wildcard can swallow implemented non-admin-worker contracts | admin secrets/auth; route specificity must win over namespace default | `admin-worker` default namespace; `immigrate-worker` bridge only for listed aliases | HIGH | LOCKED_DEFAULT_WITH_EXCEPTIONS | exceptions retained until migrated/proxied | Phase 5 | Verify bearer/session/auth per endpoint family and maintain route-level exceptions before any wildcard route migration; bridge aliases may be removed only after direct canonical clients and acceptance migration complete. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `/v1/admin/*` | `admin-worker` default owner with explicit route exceptions | internal/admin | `ADMIN_BEARER`, signed session, confirm key, or service binding per endpoint | Admin Worker and backend stores; route-specific exception Workers where documented | admin pages not all normalized; `POST /v1/admin/access/grant` remains `auth-worker`; `/v1/admin/line/liff-renewal-queue` remains non-default/pending source confirmation | possible front gate/service proxy | direct admin route/service binding; exceptions must keep exact owners until migrated/proxied | LOCKED default, exceptions retained |
...
- `POST /v1/admin/create-session`, `POST /v1/admin/create-job`, and `POST /v1/admin/jobs/create-session` are explicit temporary bridge exceptions attached to `immigrate-worker` on apex and www. They normalize JSON payloads when applicable, forward cookies plus bridge headers through the `ADMIN_WORKER` binding, strip the inbound Authorization header, and target `/v1/admin/job/create`; `admin-worker` owns the canonical backend operation but does not directly implement these aliases.
...
| `admin-worker` | canonical active API owner | Owns default `/v1/admin/*`, model session/admin APIs, exact admin login/session, Kenji Knowledge canonical shell, and canonical `/v1/admin/job/create`; route-level `/v1/admin/*` exceptions retain current ingress owners until migration/proxy. |
