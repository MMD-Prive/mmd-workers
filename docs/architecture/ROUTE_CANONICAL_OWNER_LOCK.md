...
| Admin APIs | `/v1/admin/*` | route-level exceptions: `POST /v1/admin/access/grant`, `/v1/admin/line/liff-renewal-queue` | API hosts | GET, POST | `admin-worker` default; explicit exceptions keep current owners | direct, proxy, or exception-specific route | none | `admin-worker` default; `auth-worker` owns grant; member-pages LIFF renewal queue remains exception pending source confirmation | LIVE CONNECTED | not fully enumerated here | wildcard can swallow implemented non-admin-worker contracts | admin secrets/auth; route specificity must win over namespace default | `admin-worker` default namespace | HIGH | LOCKED | exceptions retained until migrated/proxied | Phase 5 | Verify bearer/session/auth per endpoint family and maintain route-level exceptions before any wildcard route migration. |
...
| `admin-worker` | canonical active API owner | Owns default `/v1/admin/*`, model session/admin APIs, exact admin login/session, and Kenji Knowledge canonical shell; route-level `/v1/admin/*` exceptions retain current owners until migration/proxy. |
...
| `sigil-worker` | candidate target for SIGIL apply/private model APIs; active for other implemented APIs | SIGIL apply/private model apply remain UNRESOLVED until matching handlers and route attachment are proven; recovery/public model API overlap still requires future lock. |
