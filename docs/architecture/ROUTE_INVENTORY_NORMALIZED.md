...
| `/sigil/member/membership` | Worker page | LIVE CONNECTED | `mmd-redirect-worker/src/index.js` delegates the canonical SIGIL member gate to `member-pages-worker`; `member-pages-worker/src/index.js` renders membership package selection. | Member package page exists; payment proof stays evidence only; no auto-renewal redirect. | Canonical owner: `member-pages-worker`, still fronted by `mmd-redirect-worker`. |
...
| `/sigil/member/membership` | `member-pages-worker` | High | Front gate delegates the canonical SIGIL member gate to member-pages worker. |
