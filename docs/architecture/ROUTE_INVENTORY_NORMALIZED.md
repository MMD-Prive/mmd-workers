...
| `/member/payments` | Member payment status/history/navigation | TARGET LOCKED | Webflow page source is canonical presentation; target BFF is `member-pages-worker`; `payments-worker` remains money truth. | Legacy admin/front-gate delegation is migration-only until the member-safe BFF passes production acceptance. | Do not expose admin review controls or mint payment references from this page. |
...
| `/pay/membership` | Public Membership selection | LIVE CONNECTED | Webflow page `/pay/membership`; backend package/payment-intent APIs remain server-owned. | Member / Elite / Red Card only; payment status belongs to `/member/payments`; signed Public checkout is `/pay/checkout?t=...`. | Canonical Public Membership entry; never payment-status or generic proof authority. |
...
| `/pay/membership` | Webflow Public Membership presentation; backend payment truth elsewhere | High | Public Member / Elite / Red Card selection is canonical here; signed checkout remains backend-issued `/pay/checkout?t=...`. |
