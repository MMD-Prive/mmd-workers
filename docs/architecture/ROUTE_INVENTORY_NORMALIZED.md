...
| `/sigil/member/membership` | Webflow page | LIVE CONNECTED | `mmd-redirect-worker/src/index.js` keeps the route in the explicit Webflow pass-through set; the published Webflow page contains the complete Private Membership experience. | Full Webflow membership page is visible; LIFF identity API and payment truth remain Worker/backend-owned. | Canonical page owner: Webflow, still fronted by `mmd-redirect-worker`; do not replace it with the abbreviated Worker fallback. |
...
| `/sigil/pay/renewal` | Redirect-only compatibility | ROUTE ONLY / REDIRECT | Production patch in `member-dashboard-chat-worker` owns the redirect bridge. | Signed `t` -> `/sigil/pay?t=...`; unsigned -> `/sigil/member/membership?intent=renew`; no rendered fallback UI. | Keep redirect-only compatibility; never restore Renewal Payment Review UI. |
| `/pay/renewal` | Redirect-only compatibility alias | ROUTE ONLY / REDIRECT | `member-dashboard-chat-worker` renewal redirect bridge. | Same contract as `/sigil/pay/renewal`; signed `t` preserves canonical payment authority, unsigned enters Private renewal selection. | Keep redirect-only compatibility. |
...
| `/pay/membership` | Public Membership selection | LIVE CONNECTED | Webflow Public Membership presentation; backend package/payment-intent APIs. | Member / Elite / Red Card selection only; backend issues signed `/pay/checkout?t=...`. | Webflow owns presentation; `payments-worker` remains money truth. |
...
| `/public/access` | Public LINE identity / access gate | PARTIAL INTEGRATION | Webflow page `6a58a1f259f7960f6edafce9` is the current presentation source with root `#mmd-access-gate`; its primary CTA enters LINE LIFF status and its secondary CTA opens `/member/my-mmd`. `public-access-worker` separately implements `POST /public/api/access/intake`. | Presentation owner is Webflow; the visible gate does not submit the intake API and must not be treated as intake evidence. Production GET/HEAD route smoke and intake-API live smoke remain separate acceptance items. | Keep Webflow as presentation owner; keep `public-access-worker` as separate intake API implementation owner; do not couple them without a new product decision. |
...
| `/v1/apply/public-model` | API | API READY / UI MISSING | `partners-worker/wrangler.toml`; `partners-worker/src/index.ts`. | Implemented public-model application endpoint with matching custom-host routes; unversioned `/apply/public-model` alias is also implemented by `partners-worker`. No `/v1/public-model/apply` handler is proven in `sigil-worker`; do not preserve or plan around that phantom route. |
...
| `/v1/admin/clients/lineage-lookup` | API bridge | ROUTE ONLY / BACKEND MISSING | `immigrate-worker/wrangler.toml` declares apex/www exact routes; `immigrate-worker/src/internal-routes.ts` forwards `/v1/admin/*` through the `ADMIN_WORKER` binding with cookie and bridge headers; no matching `admin-worker/src/index.js` handler was found. | Preserve temporary ingress, but backend operation owner remains UNRESOLVED until a handler/contract is implemented or formally mapped. |
| `/v1/admin/clients/recent` | API bridge | ROUTE ONLY / BACKEND MISSING | `immigrate-worker/wrangler.toml` declares apex/www exact routes; `immigrate-worker/src/internal-routes.ts` forwards `/v1/admin/*` through the `ADMIN_WORKER` binding with cookie and bridge headers; no matching `admin-worker/src/index.js` handler was found. | Preserve temporary ingress, but backend operation owner remains UNRESOLVED until a handler/contract is implemented or formally mapped. |
...
| `/v1/admin/job/draft` | API bridge | ROUTE ONLY / BACKEND MISSING | `immigrate-worker/wrangler.toml` declares apex/www exact routes; `immigrate-worker/src/internal-routes.ts` forwards `/v1/admin/*` through the `ADMIN_WORKER` binding with cookie and bridge headers; no matching `admin-worker/src/index.js` handler was found. | Preserve temporary ingress, but backend operation owner remains UNRESOLVED until a handler/contract is implemented or formally mapped. |
...
| `/v1/admin/line/push` | API bridge | ROUTE ONLY / BACKEND MISSING | `immigrate-worker/wrangler.toml` declares apex/www exact routes; `immigrate-worker/src/internal-routes.ts` forwards `/v1/admin/*` through the `ADMIN_WORKER` binding with cookie and bridge headers; no matching `admin-worker/src/index.js` handler was found. | Preserve temporary ingress, but backend operation owner remains UNRESOLVED until a handler/contract is implemented or formally mapped. |
...
| `/v1/admin/model/session/link` | API | PARTIAL INTEGRATION | `admin-worker/src/index.js` implements the handler; `admin-worker/wrangler.toml` has no exact custom-host route or broad `/v1/admin/*` route for this path. | Implementation owner is `admin-worker`; production route owner remains UNRESOLVED until exact ingress or service binding is proven. |
| `/v1/model/session/current` | API | PARTIAL INTEGRATION | `admin-worker/src/index.js` implements the handler; `admin-worker/wrangler.toml` has no exact custom-host route for `/v1/model/session/current`. | Implementation owner is `admin-worker`; direct production route owner remains UNRESOLVED until exact ingress or service binding is proven. |
| `/v1/model/session/action` | API | PARTIAL INTEGRATION | `admin-worker/src/index.js` implements the handler; `admin-worker/wrangler.toml` has no exact custom-host route for `/v1/model/session/action`. | Implementation owner is `admin-worker`; direct production route owner remains UNRESOLVED until exact ingress or service binding is proven. |
| `/v1/admin/job/create` | API | PARTIAL INTEGRATION | `admin-worker/src/index.js` implements the handler; `admin-worker/wrangler.toml` has no exact custom-host route or broad `/v1/admin/*` route. Specific `immigrate-worker` aliases can forward to this backend, but direct canonical ingress is unproven. | Implementation owner is `admin-worker`; direct production route owner remains UNRESOLVED. Preserve alias ingress contracts separately. |
| `/v1/admin/members/list` | API | PARTIAL INTEGRATION | `admin-worker/src/index.js` implements the handler; no exact production route or service binding is proven in `admin-worker/wrangler.toml`. | Implementation owner is `admin-worker`; production route owner remains UNRESOLVED until exact ingress is proven. |
| `/v1/admin/members/update` | API | PARTIAL INTEGRATION | `admin-worker/src/index.js` implements the handler; no exact production route or service binding is proven in `admin-worker/wrangler.toml`. | Implementation owner is `admin-worker`; production route owner remains UNRESOLVED until exact ingress is proven. |
| `/v1/admin/models/list` | API | PARTIAL INTEGRATION | `admin-worker/src/index.js` implements the handler; no exact production route or service binding is proven in `admin-worker/wrangler.toml`. | Implementation owner is `admin-worker`; production route owner remains UNRESOLVED until exact ingress is proven. |
| `/v1/admin/models/search` | API | PARTIAL INTEGRATION | `admin-worker/src/index.js` implements the handler; direct admin-worker route is absent. A separate `immigrate-worker` ingress exception forwards the same path and must remain distinct. | Implementation owner is `admin-worker`; direct production route owner remains UNRESOLVED outside the temporary bridge ingress. |
| `/v1/admin/models/upsert` | API | PARTIAL INTEGRATION | `admin-worker/src/index.js` implements the handler; no exact production route or service binding is proven in `admin-worker/wrangler.toml`. | Implementation owner is `admin-worker`; production route owner remains UNRESOLVED until exact ingress is proven. |
| `/v1/admin/payment/proof` | API | PARTIAL INTEGRATION | `admin-worker/src/index.js` implements the handler; no exact production route or service binding is proven in `admin-worker/wrangler.toml`. | Implementation owner is `admin-worker`; production route owner remains UNRESOLVED. Do not treat frontend proof as approval. |
...
| `/v1/sigil/board/status` | API | PARTIAL INTEGRATION | `sigil-worker/src/index.js` implements read-only status; `sigil-worker/wrangler.toml` declares `workers_dev = true` and no custom routes; `webflow/sigil/board/kenji-board-v70-webflow-snippet.html` calls this same-origin path and falls back to safe-mode cards on API failure. | Implementation owner is `sigil-worker`; production/custom-host owner remains UNRESOLVED until exact route or service binding is proven. |
| `/v1/sigil/board/queue` | API | PARTIAL INTEGRATION | `sigil-worker/src/index.js` implements read-only queue; `sigil-worker/wrangler.toml` declares `workers_dev = true` and no custom routes; `webflow/sigil/board/kenji-board-v70-webflow-snippet.html` calls this same-origin path and falls back to safe-mode cards on API failure. | Implementation owner is `sigil-worker`; production/custom-host owner remains UNRESOLVED until exact route or service binding is proven. |
...
| `/telegram/webhook` and `/v1/webhook` | API | LIVE CONNECTED / LOCKED | `telegram-worker/src/index.js`; `telegram-worker/wrangler.toml`; `.github/workflows/deploy-telegram-worker.yml`. | `telegram-worker` is the canonical apex/www route owner. Deploy acceptance syncs the exact routes, requires webhook-secret enforcement, points Telegram Bot API at `https://mmdbkk.com/telegram/webhook`, and smokes invalid-secret fail-closed behavior. |
| `/telegram/internal/send`, `/v1/internal/send`, `/v1/send` | API | LIVE CONNECTED / LOCKED | `telegram-worker/src/index.js`; `telegram-worker/wrangler.toml`; `.github/workflows/deploy-telegram-worker.yml`. | `telegram-worker` is the canonical apex/www route owner for internal-send aliases. Exact-route sync refuses ownership conflicts and production smoke proves unauthenticated requests fail closed. workers.dev remains compatibility transport for existing internal callers. |
...
| `/member/membership` | Alias | `mmd-redirect-worker/src/index.js` maps to `/sigil/member/membership`. | Yes | Keep for compatibility until the SIGIL member route is production verified. |
| `/membership` | Alias | `mmd-redirect-worker/src/index.js` maps to `/sigil/member/membership`. | Yes | Remove only after member flow is production ready. |
| `/member/membership/benefits` | Alias | `mmd-redirect-worker/src/index.js` maps to `/sigil/member/membership`. | Yes | Remove only after member flow is production ready. |
| `/membership/benefits` | Alias | `mmd-redirect-worker/src/index.js` maps to `/sigil/member/membership`. | Yes | Remove only after member flow is production ready. |
...
| Public model apply | `partners-worker` exposes `/v1/apply/public-model` and `/apply/public-model`; no `/v1/public-model/apply` handler is proven in `sigil-worker`. | Normalize on the implemented partners-worker route family; do not treat `/v1/public-model/apply` as an existing SIGIL API. |
...
| `/sigil/pay/renewal` | `member-dashboard-chat-worker` redirect-only compatibility owner | High | Production redirect patch is deployed; no customer-visible renewal fallback UI. |
| `/sigil/member/membership` | Webflow | High | Front gate passes the canonical page through to the published Webflow UI; member APIs remain separate. |
| `/pay/membership` | Webflow Public Membership presentation + backend intent APIs | High | Public Member / Elite / Red Card selection; `payments-worker` remains final money truth. |
...
| `/public/access` | Webflow presentation; `public-access-worker` separate intake API owner | Medium | Webflow page `6a58a1f259f7960f6edafce9` and `#mmd-access-gate` prove the current visible access-gate source. Production route smoke and intake API smoke remain pending; the visible gate is not an intake form. |
