...
| 30 | Kenji Knowledge admin | GET, HEAD | canonical `/sigil/internal/admin/kenji-knowledge`; old `/internal/admin/kenji-knowledge` retained as `temporary_compatibility_alias`; twelve narrow admin-worker patterns | admin-worker · page | same shell; unchanged auth · Q all | yes | G,H | P0 | verify canonical apex/www/slash/query/auth/API/assets/logs before explicit alias removal approval / B1.2 |
...
PR B1.2 repository state is `prepared_not_live_verified`: canonical and alias paths share one shell implementation; six canonical patterns are added while six alias patterns remain; route-kind and canonical headers distinguish them. The global catch-alls remain unchanged. Alias removal requires canonical production deploy, authenticated apex/www smokes, API and asset MIME passes, browser acceptance, an observation window, and explicit approval. Do not mark the alias retired before that gate.
