...
- POST raw bytes to returned `upload_endpoint`. JPG/PNG/WebP ≤15 MiB; MP4 ≤25 MiB. Plans expire after 30 minutes. Worker verifies size/signature, writes an immutable R2 key and SHA-256 metadata, then creates a review request and moves the asset to `pending_review` with all safety flags false.
