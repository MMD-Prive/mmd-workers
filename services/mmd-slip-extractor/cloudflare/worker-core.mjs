...
async function extractionAuthorized(request, env) {
  ...
  if (scope === "staging") {
    return safeBearerMatch(request.headers.get("authorization"), env.MMD_SLIP_EXTRACTOR_TOKEN);
  }
  return false;
}
...
export async function handleExtractorRequest(request, env, dependencies) {
  ...
  // Production is service-binding only. Even /health must come from an approved
  // internal caller so this worker cannot become a second public payment surface.
  if (scope === "production" && !productionServiceBindingAuthorized(request)) {
    return json({ error: "not_found" }, 404, id, env);
  }

  if (isExtraction) {
    const authorized = await extractionAuthorized(request, env);
    if (!authorized) return json({ error: "unauthorized" }, 401, id, env);
