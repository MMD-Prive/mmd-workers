...
test("production accepts member-dashboard service binding without bearer auth", async () => {
  ...
  const response = await handleExtractorRequest(
    ...
    productionEnv,
    { getContainer: (binding, instanceName) => {
      assert.equal(instanceName, "production-singleton");
      return fakeContainer((request) => { forwarded = request; });
    } },
  );
  assert.equal(response.status, 200);
...
test("invalid runtime scope fails before routing", async () => {
  const response = await handleExtractorRequest(
    new Request("https://extractor.test/health"),
    { ...baseEnv, MMD_RUNTIME_SCOPE: "unknown" },
    { getContainer: () => fakeContainer() },
  );
