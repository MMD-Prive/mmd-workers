(function () {
  ...
  var COPY = {
    th: {
      "lang.aria": "เลือกภาษา",
      "chip.console": "PUBLIC MODEL CONSOLE",
      ...
      "fieldOtRate": "OT Rate / hour",
      "fieldOtAfterMidnightRate": "OT After Midnight / hour",
      "fieldExtraQueueAfter": "Extra Queue After",
      "workText": "Package เริ่มได้ตั้งแต่ 2–5 ชั่วโมง ถ้าเกินเวลาที่จองไว้จะคิด OT ตามเรทที่ทีมอนุมัติให้แต่ละคน ถ้ายังไม่ได้ตั้งค่า ระบบจะใช้เรท default ของงาน public ก่อน และถ้าเกิน 12 ชั่วโมงจะนับเป็นอีก 1 คิวทันที",
      "noticeReady": "Model-specific OT",
      ...
      "work.publicMin": "Minimum Rate",
      "work.publicStandard": "Standard Rate",
      ...
      "otRuleNote": "OT ใช้เรทที่ทีมอนุมัติให้แต่ละคน ถ้ายังไม่ตั้งค่าให้ใช้ default public ก่อน",
  ...
  COPY.en = Object.assign({}, COPY.th, {
    ...
    "fieldOtAfterMidnightRate": "OT After Midnight / hour",
    "fieldExtraQueueAfter": "Extra Queue After",
    "workText": "Package starts from 2-5 hours. Time beyond the booked package uses the model-specific, admin-approved OT rate. If no rate is set, use the public default first. Over 12 hours counts as one additional queue immediately.",
    "noticeReady": "Model-specific OT",
    "work.publicJobValue": "2-5 hour package + actual OT",
    "work.extraQueue": "Over 12 hours = one additional queue immediately",
    "work.notes": "Work Notes",
    "otRuleNote": "OT uses each model's admin-approved rate. If none is set, use the public default first.",
  ...
  COPY.zh = Object.assign({}, COPY.en, {
    ...
    "fieldOtAfterMidnightRate": "OT After Midnight / hour",
    "fieldExtraQueueAfter": "Extra Queue After",
    "workText": "Package 可从 2-5 小时开始。超过已预订时间后，OT 使用每位 model 经团队批准的专属价格。若尚未设置，先使用 public 默认价格。超过 12 小时会立即算作额外 1 个队列。",
    "noticeReady": "Model-specific OT",
    ...
    "work.notes": "工作备注",
    "otRuleNote": "OT 使用每位 model 经团队批准的价格；未设置时先用 public 默认价格。",
    "budget.title": "客户预算",
  ...
  COPY.ja = Object.assign({}, COPY.en, {
    ...
    "fieldOtAfterMidnightRate": "OT After Midnight / hour",
    "fieldExtraQueueAfter": "Extra Queue After",
    "workText": "Package は 2-5 時間から開始できます。予約時間を超えた分の OT は、各 model ごとにチーム承認済みの料金を使います。未設定なら public default を先に使います。12 時間を超える場合は、すぐに追加 1 キューとして扱います。",
    "noticeReady": "Model-specific OT",
    ...
    "work.notes": "仕事メモ",
    "otRuleNote": "OT は各 model のチーム承認済み料金を使用します。未設定の場合は public default を先に使います。",
  ...
  function createState(root, view) {
    ...
    return {
      ...
      location: trim(view.briefLocation && view.briefLocation.textContent) || "Sukhumvit / Lounge",
      mapUrl: "",
  ...
  function readExtraQueueAfter(rates) {
    var value = Number(trim(rates.extra_queue_after_hours));
  ...
  function collectWorkProfile() {
    ...
    var extraQueueAfter = readExtraQueueAfter(rates);
    ...
    return {
      ...
      public_min_rate: rates.public_min_rate || rates.public_minimum_rate || "",
      public_standard_rate: rates.public_standard_rate || "",
      public_ot_rate: rates.public_ot_rate || "",
      public_ot_after_midnight_rate: rates.public_ot_after_midnight_rate || "",
      extra_queue_after_hours: extraQueueAfter,
      ...
      public_profile: {
        ...
        public_ot_after_midnight_rate: rates.public_ot_after_midnight_rate || "",
        extra_queue_after_hours: extraQueueAfter,
        minimum_rate: rates.public_min_rate || rates.public_minimum_rate || "",
