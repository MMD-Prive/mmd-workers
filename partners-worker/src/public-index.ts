...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if ((method === "GET" || method === "HEAD") && normalizePartnerPath(url.pathname) === "/partner/form") {
      const next = new URL("/partner/apply", url.origin);
...
function renderDashboardPanel(page: string, token: string): string {
  ...
  return "<section data-partner-control-room>" +
    ...
    "<label class=\"wide pcr-private-field\"><span>Private model note · Encrypted for Partner</span><textarea name=\"private_note\" placeholder=\"ปลดล็อก Private Vault เพื่อแก้ไขโน้ต\"></textarea></label>" +
...
body[data-mmd-partner-page]{
  margin:0;
  background:
    radial-gradient(circle at 9% 2%,var(--mmdp-wine-glow),transparent 32rem),
...
body[data-mmd-partner-page] *{box-sizing:border-box}
...
.mmdp-nav a{font-family:var(--mmdp-font-en);text-decoration:none;color:var(--mmdp-muted);font-weight:600}
...
.mmdp-brand b{color:var(--mmdp-ivory);font-family:var(--mmdp-font-en);font-size:24px;font-weight:700;letter-spacing:.04em}
.mmdp-brand span{color:var(--mmdp-gold);font-family:var(--mmdp-font-en);font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.16em}
...
.mmdp-eyebrow{
  ...
  font-size:11px;font-weight:600;letter-spacing:.14em;line-height:1.4;text-transform:uppercase
}
.mmdp-hero h1{
...
.mmdp-hero p,.mmdp-grid p,.mmdp-dashboard p{color:var(--mmdp-muted);line-height:1.78}
.mmdp-actions{display:flex;gap:12px;flex-wrap:wrap;margin-top:28px}
.mmdp-btn{
...
.mmdp-btn{transition:transform .2s ease,filter .2s ease,box-shadow .2s ease}
.mmdp-btn:hover{transform:translateY(-1px);filter:brightness(1.025)}
.mmdp-btn.ghost:hover{background:rgba(215,175,103,.07);border-color:var(--mmdp-line-strong)}
.mmdp-card{
  min-height:330px;border:1px solid var(--mmdp-line);border-radius:32px;padding:30px;
...
.mmdp-card small,.mmdp-card span{color:var(--mmdp-gold-2);font-family:var(--mmdp-font-en);font-weight:600;letter-spacing:.14em;text-transform:uppercase}
.mmdp-card strong{
...
.mmdp-grid span{color:var(--mmdp-gold);font-weight:600}
...
.mmdp-metrics span{display:block;color:var(--mmdp-muted);font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.14em}
.mmdp-metrics strong{display:block;margin-top:10px;color:var(--mmdp-ivory);font-size:22px}
body[data-mmd-partner-page] .pa18-card,
...
body[data-mmd-partner-page] :is(a,button,input,textarea,select):focus-visible{outline:2px solid var(--mmdp-gold-2);outline-offset:3px}
/* FINAL PARTNER CONTRAST SAFETY */
body[data-mmd-partner-page] :is(h1,h2,h3,h4,.mmdp-brand b){
  color:var(--mmdp-ivory)!important;-webkit-text-fill-color:var(--mmdp-ivory)!important;background-image:none!important
}
body[data-mmd-partner-page] :is(.mmdp-eyebrow,.mmdp-brand span){
  color:var(--mmdp-gold-2)!important;-webkit-text-fill-color:var(--mmdp-gold-2)!important
}
...
(function () {
  ...
  function ensureStatus(form) {
    ...
    status.style.color = "var(--mmdp-muted)";
