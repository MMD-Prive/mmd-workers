...
(function () {
  ...
  function renderEarnings(data) {
    ...
    var payout = '<article class="pcr-finance-card"><div><small>PAYOUT PROFILE</small><h3>' + esc(profile.configured ? (profile.method || "Configured") : "ยังไม่ได้ตั้งค่าช่องทางรับเงิน") + '</h3><p>' + esc(profile.configured ? ((profile.account_name || "—") + " · " + (profile.account_ref_masked || "—")) : "ข้อมูลนี้เป็นข้อมูลที่ Partner เลือกแชร์กับ MMD เพื่อการจ่ายเงินเท่านั้น") + '</p></div><span class="pcr-finance-state">' + (profile.configured ? "READY" : "NOT SET") + '</span></article>';
