...
(function () {
    ...
    return fetch(path + joiner + "t=" + encodeURIComponent(token), Object.assign({}, options || {}, {referrerPolicy:"no-referrer",cache:"no-store",credentials:"omit"})).then(function (response) {
      return response.json().catch(function () { return {}; }).then(function (payload) {
        ...
        if (!response.ok || !payload.ok) {
          ...
          var messages = {vault_conflict:"ข้อมูลถูกแก้จากอีกอุปกรณ์ กรุณาสำรองแล้วโหลด Vault ใหม่ก่อนบันทึก",vault_revision_required:"กรุณาโหลด Vault ใหม่ก่อนบันทึก",job_already_closed:"งานนี้สรุปผลแล้ว",explicit_share_required:"กรุณายืนยัน Share with MMD",official_verify_required:"งานอยู่ในขั้น Official Verify · ระบบจะเปิดการยืนยันเมื่อการตรวจเสร็จ",partner_confirmation_already_final:"คำตอบของงานนี้ถูกบันทึกแล้ว · รีเฟรชเพื่อดูสถานะล่าสุด",partner_not_active:"สถานะพาร์ทเนอร์กำลังอยู่ในขั้นตรวจสอบ",partner_not_recognized:"Partner Review ของ Boss Per กำลังดำเนินการ",telegram_connect_required:"เชื่อม Telegram เพื่อรับแจ้งงาน · Dashboard ใช้งานได้ตามปกติ",telegram_binding_conflict:"กรุณาติดต่อ MMD เพื่อตรวจสอบบัญชี Telegram"};
  ...
  function jobCard(job) {
    ...
    var finalLabel = job.status === "confirmed" ? "ยืนยันงานแล้ว" : job.status === "declined" ? "บันทึกการปฏิเสธแล้ว" : "";
    var actions = finalLabel ? '<div class="pcr-job-locked" role="status"><b>' + finalLabel + '</b></div>'
      : job.confirmation_allowed === true
      ? '<div class="pcr-actions"><button type="button" data-job-action="confirm">ยืนยันงาน</button><button type="button" data-job-action="changes" class="ghost">ขอแก้ไข</button><button type="button" data-job-action="decline" class="danger">ปฏิเสธ</button></div>'
      : '<div class="pcr-job-locked" role="status"><b>กำลังเตรียมงาน</b><span>MMD กำลังตรวจข้อมูลก่อนเปิดให้ตอบรับงาน</span></div>';
    var budget = Number(job.partner_source_rate_thb);
    var budgetRow = Number.isFinite(budget) && budget > 0 ? '<div><dt>งบถึงตัว</dt><dd><b>' + esc(money(budget)) + '</b></dd></div>' : '';
    var changeAlert = job.notification_status === "pending_change"
      ? '<div class="pcr-job-change-alert" role="status"><b>รายละเอียดงานมีการเปลี่ยนแปลง</b><span>ตรวจวัน เวลา และสถานที่ล่าสุดก่อนยืนยันอีกครั้ง</span></div>'
      : '';
    var locationNote = job.location_status === "pending_customer_confirmation" ? ' <small>· รอลูกค้ายืนยัน</small>' : '';
    return '<article class="pcr-job" data-job="' + esc(job.session_record_id) + '">' +
      ...
      changeAlert +
      '<dl><div><dt>งาน</dt><dd>' + esc(job.work_type || job.work_lane || "MMD assignment") + '</dd></div>' + budgetRow + '<div><dt>สถานที่</dt><dd>' + esc(job.location) + locationNote + '</dd></div><div><dt>ลูกค้า</dt><dd>' + esc(job.client_alias) + '</dd></div></dl>' +
      actions +
  ...
  function renderModels(data) {
    ...
    target.innerHTML = models.map(modelCard).join("") + pending.map(function (item) {
      return '<article class="pcr-model pending"><div class="pcr-model-image"><span>+</span></div><div class="pcr-model-copy"><h3>' + esc(item.payload.display_name || "New model") + '</h3><p>MMD Review กำลังดำเนินการ</p><div class="pcr-meta"><span>Review</span></div></div></article>';
  ...
  function submitProfile(event) {
    ...
    if (!form.elements.share_with_mmd.checked) return setFlash("ยืนยัน Share with MMD เพื่อดำเนินการกับข้อมูลโปรไฟล์ชุดนี้","error");
  ...
  function submitSales() {
    ...
    if (!audiences.length) return setFlash("เลือกกลุ่มลูกค้าที่ต้องการให้มองเห็นอย่างน้อยหนึ่งกลุ่ม","error");
  ...
  function uploadModelFile() {
    ...
    if (!file) return setFlash("เลือกไฟล์เพื่ออัปโหลด","error");
    if (!form.elements.share_with_mmd.checked) return setFlash("ยืนยัน Share with MMD เพื่อส่งไฟล์โปรไฟล์","error");
  ...
  function removeModel() {
    if (!window.confirm("ส่งคำขอถอดโมเดลนี้ออกจากสังกัด? Canonical Model จะยังคงอยู่ในระบบ")) return;
  ...
  function addModel(event) {
    event.preventDefault(); var form=event.currentTarget;
    if (!form.elements.share_with_mmd.checked) return setFlash("ยืนยัน Share with MMD เพื่อดำเนินการต่อ","error");
...
.pcr-job-locked{display:flex;flex-direction:column;gap:4px;padding:12px 14px;border:1px solid rgba(255,230,173,.2);border-radius:14px;background:rgba(230,189,114,.07)}.pcr-job-locked b{color:var(--mmdp-gold-2)}.pcr-job-locked span{color:var(--mmdp-muted);font-size:12px}.pcr-job-change-alert{display:flex;flex-direction:column;gap:4px;margin:12px 0;padding:12px 14px;border:1px solid rgba(255,198,114,.34);border-radius:14px;background:rgba(230,154,72,.10)}.pcr-job-change-alert b{color:var(--mmdp-gold-2)}.pcr-job-change-alert span{color:var(--mmdp-copy);font-size:12px}
.pcr-telegram[data-state=required]{border-color:rgba(255,230,173,.42);background:rgba(230,189,114,.08)}.pcr-telegram small{color:var(--mmdp-gold);font-size:10px;font-weight:600;letter-spacing:.12em}
.pcr-shell{display:grid;grid-template-columns:240px minmax(0,1fr);gap:24px;margin:0 0 48px}.pcr-side{position:sticky;top:92px;align-self:start;border:1px solid var(--mmdp-line);border-radius:26px;padding:18px;background:rgba(8,6,5,.88);backdrop-filter:blur(18px)}.pcr-side button{width:100%;min-height:46px;margin:3px 0;border:0;border-radius:14px;background:transparent;color:var(--mmdp-muted);text-align:left;font-weight:600;padding:0 14px;cursor:pointer}.pcr-side button[aria-current=page]{background:rgba(230,189,114,.14);color:var(--mmdp-ivory)}.pcr-side small{display:block;color:var(--mmdp-gold);font-weight:600;letter-spacing:.14em;text-transform:uppercase;margin:8px 12px 14px}.pcr-main{min-width:0}.pcr-top{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;margin-bottom:18px}.pcr-top h2{margin:0;color:var(--mmdp-ivory);font-size:clamp(30px,5vw,52px);letter-spacing:-.055em}.pcr-top p{margin:8px 0 0}.pcr-top button,.pcr-actions button,.pcr-model button,.pcr-dialog button,.pcr-telegram button{min-height:44px;padding:0 17px;border:1px solid rgba(255,230,173,.42);border-radius:999px;background:linear-gradient(135deg,#f6e2ae 0%,#e7ca86 56%,#d4ae64 100%);color:var(--mmdp-ink);font-weight:700;cursor:pointer}.pcr-flash{position:sticky;top:78px;z-index:30;margin-bottom:14px;padding:13px 16px;border-radius:15px;background:#19140e;border:1px solid var(--mmdp-line);color:var(--mmdp-ivory)}.pcr-flash[data-tone=error]{border-color:#a74755;background:#271014}.pcr-flash[data-tone=success]{border-color:rgba(93,143,112,.58);background:rgba(46,74,57,.22)}.pcr-privacy{display:flex;gap:14px;align-items:flex-start;margin:0 0 18px;padding:16px 18px;border:1px solid var(--mmdp-line);border-radius:20px;background:rgba(215,175,103,.055)}.pcr-privacy b{display:block;color:var(--mmdp-gold-2)}.pcr-privacy p{margin:4px 0 0!important}.pcr-metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:18px}.pcr-metrics article{padding:18px;border:1px solid var(--mmdp-line);border-radius:20px;background:rgba(255,255,255,.045)}.pcr-metrics span{display:block;color:var(--mmdp-muted);font-size:12px}.pcr-metrics strong{display:block;margin-top:8px;color:var(--mmdp-ivory);font-size:22px}.pcr-telegram{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:18px;padding:17px;border:1px solid var(--mmdp-line);border-radius:20px;background:rgba(255,255,255,.04)}.pcr-telegram div{display:flex;flex-direction:column;gap:4px}.pcr-telegram span{color:var(--mmdp-muted)}.pcr-telegram i{display:grid;place-items:center;width:34px;height:34px;border-radius:50%;background:rgba(93,143,112,.24);border:1px solid rgba(93,143,112,.46);color:#cfe1d5}.pcr-section{border:1px solid rgba(255,255,255,.08);border-radius:26px;padding:22px;background:rgba(255,255,255,.035)}.pcr-section-head{display:flex;justify-content:space-between;align-items:center;gap:14px;margin-bottom:16px}.pcr-section h3{margin:0;color:var(--mmdp-ivory)}.pcr-jobs,.pcr-models{display:grid;gap:13px}.pcr-job,.pcr-model{border:1px solid rgba(255,255,255,.09);border-radius:22px;padding:18px;background:rgba(0,0,0,.2)}.pcr-job-main,.pcr-model-title{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.pcr-job h3,.pcr-model h3{margin:6px 0 0;color:var(--mmdp-ivory)}.pcr-job dl{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:16px 0}.pcr-job dl div{padding:12px;border-radius:14px;background:rgba(255,255,255,.04)}.pcr-job dt{color:var(--mmdp-muted);font-size:11px}.pcr-job dd{margin:5px 0 0;color:var(--mmdp-ivory)}.pcr-status,.pcr-meta span{display:inline-flex;padding:5px 9px;border:1px solid var(--mmdp-line);border-radius:999px;color:var(--mmdp-gold-2);font-size:11px;font-weight:600}.pcr-actions{display:flex;gap:8px;flex-wrap:wrap}.pcr-actions .ghost{background:rgba(255,255,255,.025);border-color:var(--mmdp-line);color:var(--mmdp-gold-2)}.pcr-actions .danger{background:rgba(160,45,60,.16);border-color:rgba(220,85,104,.38);color:#ffc8d0}.pcr-private-field{display:block;margin-top:14px}.pcr-private-field span{display:block;margin-bottom:7px;color:var(--mmdp-gold-2);font-size:12px}.pcr-private-field textarea,.pcr-dialog input,.pcr-dialog textarea,.pcr-dialog select,.pcr-vault input,.pcr-vault textarea{width:100%;border:1px solid rgba(255,255,255,.12);border-radius:13px;background:#090705;color:var(--mmdp-ivory);padding:12px}.pcr-model{display:grid;grid-template-columns:112px minmax(0,1fr);gap:17px}.pcr-model-image{aspect-ratio:4/5;border-radius:16px;overflow:hidden;background:linear-gradient(145deg,#332414,#0d0905);display:grid;place-items:center;color:var(--mmdp-gold);font-size:36px}.pcr-model-image img{width:100%;height:100%;object-fit:cover}.pcr-model-copy p{color:var(--mmdp-muted)}.pcr-meta{display:flex;gap:7px;flex-wrap:wrap;margin:12px 0}.pcr-switch{padding:6px 9px;border-radius:999px;background:#3c171b;color:#ffcbd1;font-size:11px;font-weight:700}.pcr-switch.is-on{background:rgba(93,143,112,.20);border:1px solid rgba(93,143,112,.38);color:#d7e6dc}.pcr-table{overflow:auto}.pcr-table table{width:100%;border-collapse:collapse;min-width:680px}.pcr-table th,.pcr-table td{padding:13px;border-bottom:1px solid rgba(255,255,255,.08);text-align:left}.pcr-table th{color:var(--mmdp-gold);font-size:11px;text-transform:uppercase}.pcr-empty{padding:34px;text-align:center;color:var(--mmdp-muted)}.pcr-dialog{width:min(760px,calc(100% - 24px));max-height:92vh;border:1px solid var(--mmdp-line);border-radius:25px;background:#0d0a07;color:var(--mmdp-ivory);padding:0;box-shadow:0 40px 120px #000}.pcr-dialog::backdrop{background:rgba(0,0,0,.76);backdrop-filter:blur(8px)}.pcr-dialog-head{position:sticky;top:0;z-index:2;display:flex;align-items:center;justify-content:space-between;padding:18px 22px;background:#0d0a07;border-bottom:1px solid rgba(255,255,255,.08)}.pcr-dialog-head h2{margin:0}.pcr-dialog-head button{width:38px;height:38px;padding:0;background:transparent;color:var(--mmdp-ivory)}.pcr-form{padding:22px}.pcr-form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:13px}.pcr-form label{display:block}.pcr-form label>span{display:block;margin-bottom:7px;color:var(--mmdp-muted);font-size:12px}.pcr-form .wide{grid-column:1/-1}.pcr-form textarea{min-height:90px;resize:vertical}.pcr-form fieldset{grid-column:1/-1;border:1px solid rgba(255,255,255,.09);border-radius:17px;padding:15px}.pcr-form legend{color:var(--mmdp-gold);padding:0 6px}.pcr-checks{display:flex;flex-wrap:wrap;gap:10px}.pcr-checks label{display:flex;align-items:center;gap:7px}.pcr-form-actions{display:flex;gap:9px;flex-wrap:wrap;margin-top:18px}.pcr-form-actions .ghost{background:rgba(255,255,255,.025);border-color:var(--mmdp-line);color:var(--mmdp-gold-2)}.pcr-form-actions .danger{margin-left:auto;background:#40151c;color:#ffd3d8}.pcr-consent{display:flex!important;align-items:flex-start;gap:9px;padding:14px;border:1px solid var(--mmdp-line);border-radius:15px;background:rgba(215,175,103,.055)}.pcr-consent input{width:auto;margin-top:4px}.pcr-vault{max-width:720px}.pcr-vault-card{padding:20px;border:1px solid var(--mmdp-line);border-radius:20px;background:linear-gradient(145deg,rgba(215,175,103,.07),rgba(49,16,21,.10))}.pcr-vault-card h3{color:var(--mmdp-gold-2)}.pcr-vault-actions{display:flex;gap:8px;margin-top:10px}.pcr-vault-open textarea{min-height:220px}.pcr-vault code{color:var(--mmdp-gold-2)}.pcr-nav-mobile{display:none}.pcr-loading{padding:20px;color:var(--mmdp-muted)}
