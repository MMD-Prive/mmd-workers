...
const SIGIL = {
  ...
  modelImage: "/api/sigil/models/image",
  modelDetail: "/api/sigil/models/detail",
  legacyModelDetail: "/sigil/api/models/detail",
...
const MODEL_BY_ID_PATH_RE = /^\/api\/sigil\/models\/([^/]+)$/;
...
function isSigilBookingApiRoute(pathname: string): boolean {
  return pathname === SIGIL.clientResolve
    || pathname === SIGIL.canonicalModelSearch
    || pathname === SIGIL.modelSearch
    || pathname === SIGIL.newArrivals
    || pathname === SIGIL.modelImage
    || pathname === SIGIL.modelDetail
    || pathname === SIGIL.legacyModelDetail
    || MODEL_BY_ID_PATH_RE.test(pathname);
}
...
function fieldStringList(fields: Record<string, unknown>, keys: string[]): string[] {
  for (const key of keys) {
    const value = fields[key];
...
function isModelBookable(fields: Record<string, unknown>, scope: string): boolean {
  const status = fieldString(fields, ["status", "Status", "Model Status", "model_status"]).toLowerCase();
  const visibility = fieldString(fields, ["booking_visibility", "Booking Visibility", "visibility", "Profile Visibility"]).toLowerCase();
...
function stablePublicHttpsUrl(value: string): string {
...
function modelCoverUrl(fields: Record<string, unknown>): { cover_url: string; asset_status: string } {
  const direct = stablePublicHttpsUrl(fieldString(fields, [
    "cover_url",
    "Cover URL",
    "public_image_url",
    "Public Image URL",
    "preview_image_url",
    "Preview Image URL",
    "profile_public_image_url",
    "Profile Public Image URL",
    "mmd_public_image_url",
    "MMD Public Image URL",
  ]));

  if (direct) return { cover_url: direct, asset_status: "ready" };
  return { cover_url: "", asset_status: "drive_pending_sync" };
}

function publicServiceFitTags(fields: Record<string, unknown>): string[] {
  const allowed = new Set(["straight-fit", "gay-fit", "private-fit", "travel-fit"]);
  return Array.from(new Set(fieldStringList(fields, [
    "service_fit_tags",
    "Service Fit Tags",
    "client_fit_tags",
    "Client Fit Tags",
...
function publicModelIdentifier(fields: Record<string, unknown>, displayName: string): string {
  const raw = fieldString(fields, [
    "model_id",
    "Model ID",
    "model_record_id",
  ...
  const candidate = raw || displayName;
  return candidate
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
function publicSafeModel(record: { record_id: string; fields: Record<string, unknown> }) {
  const fields = record.fields;
  const assets = modelCoverUrl(fields);
  const displayName = fieldString(fields, [
    "display_name",
    "Display Name",
    "Model Name",
  ...
  return {
    model_id: publicModelIdentifier(fields, displayName),
    display_name: displayName,
    status: fieldString(fields, ["status", "Status", "Model Status", "model_status"]) || "active",
    cover_url: assets.cover_url,
    gallery_count: fieldNumber(fields, [
      "gallery_count",
      "Gallery Count",
      "public_gallery_count",
      "Public Gallery Count",
      "approved_gallery_count",
      "Approved Gallery Count",
    ]),
    service_fit_tags: publicServiceFitTags(fields),
...
async function handleSigilModelSearch(request: Request, env: Env): Promise<Response> {
  ...
  let match = null;
  try {
    match = await searchBookingModel(env, query);
  } catch {
    console.warn("sigil model search public lookup unavailable");
  }
  if (!match || !isModelBookable(match.fields as Record<string, unknown>, scope)) {
    return bookingJson(request, {
      ...
      matched: false,
      models: [],
      privateAllowed: Boolean(access.can_search_private_models),
    });
  }

  const model = publicSafeModel({
...
async function handleSigilModelDetail(request: Request, env: Env): Promise<Response> {
  if (request.method === "OPTIONS") {
    return withBookingCors(request, new Response(null, { status: 204 }));
  }
  if (request.method !== "GET") {
    return bookingJson(request, { ok: false, status: "method_not_allowed", error: "METHOD_NOT_ALLOWED" }, { status: 405, headers: { allow: "GET, OPTIONS" } });
  }
  const url = new URL(request.url);
  const modelId = url.searchParams.get("model_id") || url.searchParams.get("id") || url.searchParams.get("q") || "";
  if (!modelId.trim()) {
    return bookingJson(request, { ok: false, status: "validation_error", error: "MODEL_ID_REQUIRED" }, { status: 400 });
  }
  const match = await searchBookingModel(env, modelId);
  if (!match || !isModelBookable(match.fields as Record<string, unknown>, "public")) {
    return bookingJson(request, { ok: false, status: "model_not_found", reason: "not_found" }, { status: 404 });
  }
  return bookingJson(request, {
    ok: true,
    status: "model_found",
    model: publicSafeModel({
      record_id: match.record_id,
      fields: match.fields as Record<string, unknown>,
    }),
  });
...
async function handleSigilNewArrivals(request: Request, env: Env): Promise<Response> {
  if (request.method === "OPTIONS") {
    return withBookingCors(request, new Response(null, { status: 204 }));
  }
  if (request.method !== "GET") {
  ...
  let records: Array<{ record_id: string; fields: Record<string, unknown> }> = [];
  try {
    records = await listBookingModels(env, 100) as Array<{ record_id: string; fields: Record<string, unknown> }>;
  } catch {
    console.warn("sigil new-arrivals public list unavailable");
  }
  const models = records
    .filter((record) => isModelBookable(record.fields, "public"))
    .map((record) => publicSafeModel(record))
    .slice(0, 12);
  return bookingJson(request, {
    ok: true,
    status: models.length > 0 ? "models_found" : "no_models_found",
    matched: models.length > 0,
    models,
    privateAllowed: false,
  });
}

async function handleSigilModelImage(request: Request, env: Env): Promise<Response> {
  if (request.method === "OPTIONS") {
    return withBookingCors(request, new Response(null, { status: 204 }));
  }
  if (request.method !== "GET") {
  ...
  const url = new URL(request.url);
  const modelId = url.searchParams.get("model_id") || url.searchParams.get("id") || "";
  if (!modelId.trim()) {
    return bookingJson(request, { ok: false, status: "validation_error", error: "MODEL_ID_REQUIRED" }, { status: 400 });
  }
  const match = await searchBookingModel(env, modelId);
  if (!match || !isModelBookable(match.fields as Record<string, unknown>, "public")) {
    return bookingJson(request, { ok: false, status: "model_not_found" }, { status: 404 });
  }
  const assets = modelCoverUrl(match.fields as Record<string, unknown>);
  if (!assets.cover_url) {
    return bookingJson(request, { ok: false, status: "image_unavailable", asset_status: assets.asset_status }, { status: 404 });
  }
  return new Response(null, {
    status: 302,
    headers: {
      location: assets.cover_url,
      "cache-control": "public, max-age=86400",
      "x-mmd-asset-status": assets.asset_status,
    },
  });
}

async function handleSigilModelById(request: Request, env: Env, modelId: string): Promise<Response> {
  const url = new URL(request.url);
  url.searchParams.set("model_id", modelId);
  return handleSigilModelDetail(new Request(url.toString(), request), env);
}
...
async function handlePublicBookingRequest(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    return publicJson(request, env, {
      ...
      record_id: result?.migration_id || requestId,
      next_url: nextUrl,
      meta,
    });
  } catch (error) {
    return publicJson(
      ...
      env,
      {
        ...
        error: {
          ...
          message: "Booking request could not be saved for review",
        },
        meta,
      },
      { status: 502 },
      ...
      <section class="sigil-booking__hero" aria-labelledby="sigil-booking-title">
        <div class="sigil-booking__hero-grid">
          <div>
            ...
            <p class="sigil-booking__copy">Send a private booking request through the clean member gate. Choose a standard model search when the client knows the lane, or assisted request when Per should shape the recommendation.</p>
          </div>
    ...
    <script>
      (() => {
        ...
        form.addEventListener("submit", async (event) => {
          ...
          if (!customerName || !contact || !jobType || !dateParts.date || !dateParts.time || !endTime || !locationName || !Number.isFinite(durationHours) || durationHours <= 0 || !Number.isFinite(totalAmount) || totalAmount <= 0) {
          ...
          const payload = compactObject({
            ...
            build_marker: "booking-lane-standard-plus-assisted-20260516c",
            booking_lane: lane,
            request_mode: lane === "assisted" ? "assisted_request" : "standard_search",
            ...
            job_type: jobType,
            package_tier: jobType,
            duration_hours: durationHours,
            budget: totalAmount,
            booking_date: dateParts.date,
          ...
          try {
            ...
            setStatus("Booking request received for review.", "success");
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === SIGIL.canonicalModelSearch || url.pathname === SIGIL.modelSearch) {
        return await handleSigilModelSearch(request, env);
      }

      if (url.pathname === SIGIL.newArrivals) {
        return await handleSigilNewArrivals(request, env);
      }

      if (url.pathname === SIGIL.modelDetail || url.pathname === SIGIL.legacyModelDetail) {
        return await handleSigilModelDetail(request, env);
      }

      if (url.pathname === SIGIL.modelImage) {
        return await handleSigilModelImage(request, env);
      }

      const modelByIdMatch = url.pathname.match(MODEL_BY_ID_PATH_RE);
      if (modelByIdMatch?.[1]) {
        return await handleSigilModelById(request, env, decodeURIComponent(modelByIdMatch[1]));
      }
