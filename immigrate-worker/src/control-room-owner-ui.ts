...
function canonicalizeOwnerControlRoom(html: string): string {
  return html
    ...
    .replace('</head>', `${MMD_OPERATIONS_STYLE}${CONTROL_ROOM_V2_STYLE}${OWNER_ANALYTICS_STYLE}</head>`)
    .replace('<section class="view" data-view="today">', `<section class="view" data-view="today">${CONTROL_ROOM_V2_PANEL}${OWNER_ANALYTICS_PANEL}`)
    ...
    .replace("</body>", `${CONTROL_ROOM_CANON_SCRIPT}${CONTROL_ROOM_V2_SCRIPT}${OWNER_ANALYTICS_SCRIPT}${AI_OPS_SCRIPT}${SAFETY_LOCATION_UI_SCRIPT}</body>`);
}
