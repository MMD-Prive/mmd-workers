...
const CONTROL_ROOM_V2_SCRIPT = `<script data-mmd-control-room-v2-script="system-health-v1">(function(){'use strict';var root=document.querySelector('[data-mmd-control-room-v2]');if(!root)return;var q=function(s){return root.querySelector(s)},esc=function(v){return String(v==null?'':v).replace(/[&<>"']/g,function(ch){return({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'})[ch]})};function label(s){return s==='ok'?'OK':s==='action_needed'?'ACTION NEEDED':'DEGRADED'}function evidence(v){return v==='live'?'LIVE':v==='production_acceptance'?'PROD ACCEPTED':String(v||'OBSERVED').replace(/_/g,' ').toUpperCase()}function render(d){var v=d&&d.control_room_v2;if(!v)return;var overall=q('[data-v2-overall]');if(overall){overall.dataset.status=v.overall_status||'degraded';overall.textContent=label(v.overall_status)}var grid=q('[data-v2-systems]');if(grid&&Array.isArray(v.systems)){grid.innerHTML=v.systems.map(function(x){var tag=x.href?'a':'div',href=x.href?' href="'+esc(x.href)+'"':'';return'<'+tag+' class="v2card" data-status="'+esc(x.status||'degraded')+'"'+href+'><div class="v2top"><span class="v2name">'+esc(x.label||x.key)+'</span><span class="v2state">'+label(x.status)+'</span></div><span class="v2evidence">'+evidence(x.evidence_type)+'</span><div class="v2detail">'+esc(x.detail||x.source||'')+'</div></'+tag+'>'}).join('')}var release=q('[data-v2-release]');if(release){var m=v.release&&v.release.admin_worker;release.textContent=v.refresh_mode==='live_on_demand'?'LIVE REFRESH · '+(m&&m.id?('admin '+String(m.id).slice(0,8)):'runtime verified'):'baseline · production receipts'}var watch=q('[data-v2-watch]');if(watch&&v.operational_watch)watch.textContent=v.operational_watch.detail||'AI Ops on demand'}document.documentElement.dataset.mmdControlRoomV2='system-health-v1'}document.addEventListener('mmd:control-room:dashboard',function(e){render(e.detail)});if(window.__mmdControlRoomDashboard)render(window.__mmdControlRoomDashboard);var refresh=q('[data-v2-refresh]');if(refresh)refresh.onclick=async function(){refresh.disabled=true;refresh.textContent='REFRESHING';var overall=q('[data-v2-overall]');if(overall)overall.textContent='CHECKING';try{var res=await fetch('/v1/admin/dashboard?system_health=live',{credentials:'include',headers:{Accept:'application/json'},cache:'no-store'});if(res.status===401||res.status===403){location.href='/internal/admin/login?next='+encodeURIComponent(location.pathname);return}if(!res.ok)throw new Error('live_health_unavailable');var d=await res.json();window.__mmdControlRoomDashboard=d;render(d)}catch(e){if(overall){overall.dataset.status='degraded';overall.textContent='DEGRADED'}}finally{refresh.disabled=false;refresh.textContent='REFRESH LIVE'}};var open=q('[data-v2-open-ai]');if(open)open.onclick=function(){var b=document.querySelector('.mmd-aiops-btn');if(b)b.click()}})();</script>`;
