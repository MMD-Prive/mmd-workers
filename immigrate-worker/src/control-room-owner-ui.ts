...
[data-mmd-owner-actions] .oaqcoverage{display:flex;gap:6px;flex-wrap:wrap;margin:10px 0 0}.oaqcov{padding:5px 7px;border:1px solid #3b3327;border-radius:999px;color:#a89b89;font-size:7px;font-weight:850}.oaqcov[data-state="connected"]{border-color:#29533b;color:#aee2bf}.oaqcov[data-state="partial"]{border-color:#645126;color:#f1d18b}.oaqcov[data-state="unavailable"]{border-color:#6a3333;color:#ffb4b4}
...
const OWNER_ACTIONS_PANEL = `<section data-mmd-owner-actions="v1" aria-label="Owner actions">
  ...
  <div class="oaqcoverage" data-oaq-coverage aria-label="Source availability"></div><div class="oaqgrid"><div class="oaqlist" data-oaq-list><div class="oaqempty">กำลังอ่าน Owner Actions…</div></div><article class="oaqdetail" data-oaq-detail><div class="oaqlabel">DETAIL</div><h3>เลือกรายการเพื่อดูบริบท</h3><p>รายละเอียดจะบอกเหตุผล authority และขอบเขตการตัดสินใจ โดยไม่แสดงข้อมูลรายบุคคล</p></article></div>
  <div class="oaqfoot">Read-only · ไม่มีการส่งข้อความ · ไม่มีการอนุมัติหรือเปลี่ยน Business Truth</div>
</section>`;

const OWNER_ACTIONS_SCRIPT = `<script data-mmd-owner-actions-script="v1">(function(){'use strict';var root=document.querySelector('[data-mmd-owner-actions]');if(!root)return;var q=function(s){return root.querySelector(s)},esc=function(v){return String(v==null?'':v).replace(/[&<>"']/g,function(ch){return({'<':'&lt;','>':'&gt;','&':'&amp;','"':'&quot;',"'":'&#39;'})[ch]})},safeHref=function(v){v=String(v||'');return v.indexOf('/internal/')===0?v:'/internal/admin/control-room'},state=q('[data-oaq-state]'),list=q('[data-oaq-list]'),detail=q('[data-oaq-detail]'),coverage=q('[data-oaq-coverage]'),detailSeq=0;function setState(label,kind){if(state){state.textContent=label;state.dataset.state=kind||'loading'}}function renderCoverage(items){if(!coverage)return;coverage.innerHTML=(Array.isArray(items)?items:[]).map(function(x){var st=['connected','partial','unavailable'].indexOf(String(x&&x.state||''))>=0?x.state:'unavailable',count=Number(x&&x.action_count)||0;return'<span class="oaqcov" data-state="'+esc(st)+'">'+esc(x&&x.label||x&&x.source||'source')+' · '+esc(st.toUpperCase())+(count?' · '+esc(count):'')+'</span>'}).join('')}function renderDetail(d){var a=d&&d.action,x=d&&d.drilldown;if(!a||!x)return;detail.innerHTML='<div class="oaqlabel">'+esc(x.urgency||'attention').toUpperCase()+' · '+esc(x.authority||'owner')+'</div><h3>'+esc(a.title||'Owner action')+'</h3><p>'+esc(x.reason||a.summary||'')+'</p><div class="oaqfacts"><div class="oaqfact"><small>ต้องดู</small><b>'+esc(x.observed_count) +' รายการ</b></div><div class="oaqfact"><small>AUTHORITY</small><b>'+esc(x.authority||'—')+'</b></div></div><div class="oaqlabel" style="margin-top:10px">ขอบเขตการตัดสินใจ</div><p>'+esc(x.decision_boundary||'เปิดหน้าต้นทางเพื่อตรวจต่อ')+'</p><a class="oaqsource" href="'+esc(safeHref(x.source_surface))+'">เปิดหน้าต้นทาง ↗</a>'}async function open(action,button){if(!action||!action.detail_href)return;var seq=++detailSeq;Array.prototype.forEach.call(root.querySelectorAll('.oaqrow'),function(x){x.classList.toggle('is-active',x===button)});detail.innerHTML='<div class="oaqempty">กำลังอ่านรายละเอียด…</div>';try{var r=await fetch(action.detail_href,{credentials:'include',cache:'no-store',headers:{accept:'application/json'}});if(seq!==detailSeq)return;if(r.status===401||r.status===403){setState('OWNER ONLY','restricted');detail.innerHTML='<div class="oaqempty">รายละเอียดนี้เปิดได้เฉพาะ Owner session</div>';return}var d=await r.json().catch(function(){return null});if(seq!==detailSeq)return;if(!r.ok||!d||d.ok!==true)throw new Error('detail_unavailable');renderDetail(d)}catch(_){if(seq!==detailSeq)return;detail.innerHTML='<div class="oaqempty">ยังอ่านรายละเอียดไม่ได้ · ลองโหลดใหม่อีกครั้ง</div>'}}function render(d){var actions=Array.isArray(d&&d.actions)?d.actions:[],unavailable=Array.isArray(d&&d.unavailable_sources)?d.unavailable_sources.filter(Boolean):[];renderCoverage(d&&d.source_coverage);setState(actions.length?'READY':unavailable.length?'PARTIAL':'CLEAR', actions.length?'ready':unavailable.length?'restricted':'ready');if(!actions.length){list.innerHTML='<div class="oaqempty">'+(unavailable.length?'ยังอ่านบาง source ไม่ได้: '+esc(unavailable.join(', ')):'ตอนนี้ยังไม่มีรายการที่ต้องดู')+'</div>';return}list.innerHTML=actions.map(function(a){return'<button type="button" class="oaqrow" data-key="'+esc(a.action_key)+'"><span><strong>'+esc(a.title||'รายการที่ต้องดู')+'</strong><small>'+esc(a.summary||'')+'</small></span><span class="oaqcount">'+esc(a.count)+'</span></button>'}).join('');Array.prototype.forEach.call(list.querySelectorAll('.oaqrow'),function(b){b.onclick=function(){var key=b.getAttribute('data-key');open(actions.filter(function(a){return a.action_key===key})[0],b)}})}async function load(){try{var r=await fetch('/v1/admin/dashboard/owner-actions',{credentials:'include',cache:'no-store',headers:{accept:'application/json'}});if(r.status===401||r.status===403){setState('OWNER ONLY','restricted');list.innerHTML='<div class="oaqempty">Owner Actions เปิดได้เฉพาะ Owner session</div>';return}var d=await r.json().catch(function(){return null});if(!r.ok||!d||d.ok!==true)throw new Error('queue_unavailable');render(d)}catch(_){setState('UNAVAILABLE','restricted');list.innerHTML='<div class="oaqempty">ยังอ่าน Owner Actions ไม่ได้ · ไม่แสดงข้อมูลเดาแทน</div>'}}load()})();</script>`;
