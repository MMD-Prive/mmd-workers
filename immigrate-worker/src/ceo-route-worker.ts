...
const CEO_DASHBOARD = "/internal/ceo/dashboard";
...
export async function serveCeoRoute(request: Request): Promise<Response> {
  ...
  if (incoming.pathname === CEO_ROOT) {
    return redirectToCeoDashboard(request);
  }
  ...
  if (incoming.pathname === CEO_DASHBOARD) {
...
function redirectToCeoDashboard(request: Request): Response {
  const target = new URL(request.url);
  target.pathname = CEO_DASHBOARD;
  const headers = new Headers({
    location: target.toString(),
    "cache-control": "no-store",
    "x-mmd-ceo-route-owner": "immigrate-worker",
    "x-mmd-ceo-alias": "root-to-dashboard",
    "x-mmd-ceo-human-operator": "per",
    "x-mmd-ceo-mode": "single-owner-v1",
  });
