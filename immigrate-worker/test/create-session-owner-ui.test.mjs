...
try {
  ...
  assert.equal(response.headers.get("x-mmd-create-session-interface"), "guided-simple-v2");
  assert.match(body, /class="mmd-cs-v14"/);
  assert.match(body, /data-mmd-create-session-pro/);
  assert.match(body, />เปิดงานใหม่</);
  ...
  assert.match(body, /id="gate-panel"/);
  assert.match(body, /data-op-folder-grid/);
  assert.match(body, /data-op-model-select/);
  assert.match(body, /data-op-line-user-id/);
  assert.match(body, /data-op-human-assistant/);
  assert.match(body, /data-op-escalation-owner value="Boss Per"/);
  ...
  assert.doesNotMatch(body, /Ewvon/);
