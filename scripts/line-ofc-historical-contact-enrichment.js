...
const F = {
  ...
  console: { uid:"fldizotASR8QgtSVK", display:"fldCL1LmqhbZdBE8J", lineId:"fldqtM7aoe9i8O0Ya", email:"fldUraGV8yhE2yW9n", phone:"fldeYBg1ajvvbbiW8", tg:"fld4QeRDSuy2CFKlk", raw:"fld9NbPo1Q3E6MfB8", payload:"fldn7MS0D9cdyQLtF", client:"fld9gMhvFWegM345O" },
...
function buildProfiles(clients,current,old,consoleRows){
  ...
  function enrich(rows,S){for(const r of rows){const f=r.fields||{},targets=new Set(links(f[S.client])),uid=clean(f[S.uid],80);if(REAL_LINE.test(uid)&&byLine.has(uid))targets.add(byLine.get(uid));for(const id of targets){const p=profiles.get(id);if(!p)continue;for(const k of [S.display,S.rename])if(k&&goodName(f[k]))p.names.add(normName(f[k]));if(S.email){const e=email(f[S.email]);if(e)p.emails.add(e)}if(S.phone){const ph=phone(f[S.phone]);if(ph)p.phones.add(ph)}if(S.tg){const h=handle(f[S.tg]);if(h)p.handles.add(h)}if(S.lineId){const h=handle(f[S.lineId]);if(h)p.handles.add(h)}for(const e of extractEmails(S.raw?f[S.raw]:""))p.emails.add(e);for(const e of extractEmails(S.payload?f[S.payload]:""))p.emails.add(e);for(const ph of extractPhones(S.raw?f[S.raw]:""))p.phones.add(ph);for(const ph of extractPhones(S.payload?f[S.payload]:""))p.phones.add(ph);}}
  }
  enrich(current,F.current);enrich(old,F.old);enrich(consoleRows,F.console);
...
async function run({apply=false,reportPath="",assertIdempotent=false}={}){
  const A=new Airtable(), vals=o=>Object.values(o);const [clients,members,current,old,consoleRows,evidence]=await Promise.all([A.list(T.clients,vals(F.clients)),A.list(T.members,vals(F.members)),A.list(T.current,vals(F.current)),A.list(T.old,vals(F.old)),A.list(T.console,vals(F.console)),A.list(T.evidence,vals(F.evidence))]);const ctx=buildProfiles(clients,current,old,consoleRows), clientById=new Map([...ctx.profiles].map(([id,p])=>[id,p.record])), membersByClient=new Map();for(const m of members)for(const id of links(m.fields?.[F.members.clients])){if(!membersByClient.has(id))membersByClient.set(id,[]);membersByClient.get(id).push(m)}
