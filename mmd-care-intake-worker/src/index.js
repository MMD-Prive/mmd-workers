  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/ping" || url.pathname === "/health") {
        ...
        return json({ ok: true, worker: workerName(env), mode: MODE, status_path: STATUS_PATH }, 200, cors);
      }
...
async function handleComplaintStatus(request, env, cors) {
  ...
  if (!id && !sid && !token) return json({ ok: false, error: "missing_lookup", message: "Complaint ID, session_id, or token is required" }, 400, cors);
...
async function writeCase(env, record) {
  ...
  const keys = [`${CASE_KEY_PREFIX}id:${record.complaint_id}`, record.session_id ? `${CASE_KEY_PREFIX}sid:${record.session_id}` : "", record.token ? `${CASE_KEY_PREFIX}token:${shortHash(record.token)}` : ""].filter(Boolean);
  try {
...
async function findKvCase(env, lookup) {
  ...
  const keys = [lookup.id ? `${CASE_KEY_PREFIX}id:${lookup.id}` : "", lookup.sid ? `${CASE_KEY_PREFIX}sid:${lookup.sid}` : "", lookup.token ? `${CASE_KEY_PREFIX}token:${shortHash(lookup.token)}` : ""].filter(Boolean);
...
function boardCard(record) {
  ...
  return { id: `sigil_card_${shortHash(record.complaint_id)}`, title: `Private care complaint: ${clean(titleName, "new case", 60)}`, lane: record.lane === "model" ? "Model" : "Private Review", status: "Need Info", priority, risk: riskFor(record), next_action: "MMD Assistant ตรวจข้อมูลและหลักฐานก่อนส่งต่อ Per หากจำเป็น", owner: record.lane === "internal" ? "Kenji" : "MMD", needs_per_decision: priority === "Critical", summary: clean(record.lane_statement || record.statement || "Complaint evidence received", "Complaint evidence received", 180), received_at: record.received_at };
}
...
function publicCaseFromAirtable(record) {
  ...
  return publicCaseFromRecord({ complaint_id: fields.complaint_id, session_id: fields.session_id, lane: fields.lane || "Private Review", workflow_status: fields.status, priority: fields.priority, risk: fields.risk, received_at: fields.received_at, updated_at: fields.updated_at, evidence: { total_files: Number(fields.evidence_count || fields.r2_stored_files || 0) }, board_card_id: fields.board_card_id, next_action: fields.next_action, needs_per_decision: fields.needs_per_decision, public_status_note_th: fields.public_status_note_th, public_status_note_en: fields.public_status_note_en });
}

function publicCaseFromRecord(record) {
  return { complaint_id: record.complaint_id, session_id: record.session_id || null, status: normalizeWorkflowStatus(record.workflow_status || record.status), lane: record.lane === "model" ? "Model" : "Private Review", priority: record.priority || priorityFor(record), risk: record.risk || riskFor(record), received_at: record.received_at || null, updated_at: record.updated_at || record.received_at || null, evidence_count: Number(record.evidence?.total_files || record.evidence_count || 0), board_card_id: record.board_card_id || (record.complaint_id ? `sigil_card_${shortHash(record.complaint_id)}` : ""), next_action: record.next_action || "MMD Assistant ตรวจข้อมูลและหลักฐานก่อนส่งต่อ Per หากจำเป็น", needs_per_decision: Boolean(record.needs_per_decision), public_status_note_th: record.public_status_note_th || "Kenji รับเรื่องไว้ในระบบแล้วครับ และ MMD จะตรวจข้อมูลอย่างระวัง", public_status_note_en: record.public_status_note_en || "Kenji has received this case and MMD will review the details carefully." };
}
...
function airtableConfig(env) {
  ...
  return { enabled: true, token, baseId: env?.AIRTABLE_BASE_ID || AIRTABLE_BASE_ID_DEFAULT, tableName: env?.AIRTABLE_PRIVATE_CARE_CASES_TABLE || AIRTABLE_CASE_TABLE_DEFAULT };
}

async function airtableFetch(env, path, init = {}) {
  const airtable = airtableConfig(env);
  const res = await fetch(`https://api.airtable.com/v0${path}`, { ...init, headers: { authorization: `Bearer ${airtable.token}`, "content-type": "application/json", ...(init.headers || {}) } });
...
async function forwardWebhooks(env, record) {
  return { google_drive: await postJson(env?.COMPLAINT_GOOGLE_DRIVE_WEBHOOK_URL, record, "google_drive"), telegram: await postJson(env?.COMPLAINT_TELEGRAM_WEBHOOK_URL, telegramPayload(record), "telegram") };
}
...
function corsHeaders(request, env) {
  ...
  return { "access-control-allow-origin": origin && allowed.has(origin) ? origin : "*", "access-control-allow-methods": "GET, POST, OPTIONS", "access-control-allow-headers": "content-type, authorization, x-mmd-client, x-mmd-route, x-confirm-key", "access-control-max-age": "86400", vary: "Origin" };
}
...
function normalizeLane(value) {
  ...
  if (lane === "assistant") return "internal";
...
function clean(value, fallback = "", maxLength = 180) {
  ...
  output = output.replace(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi, "[masked]").replace(/\+?\d[\d\s().-]{7,}\d/g, "[masked]").replace(/\bU[a-f0-9]{20,}\b/gi, "[masked]").replace(/\b\d{7,}:[A-Za-z0-9_-]{20,}\b/g, "[masked]").replace(/https?:\/\/\S+/gi, "[masked]").replace(/\b(token|secret|passphrase|api[_ -]?key|bank|slip[_ -]?url)\b/gi, "[redacted]");
