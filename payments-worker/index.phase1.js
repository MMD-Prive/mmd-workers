...
export default {
  async fetch(request, env, ctx) {
    ...
    // Suppress the legacy per-payment floor(amount/rate) writer inside index.js.
    // The response/payment/session handling still runs unchanged. Phase 1 writes
    // the only new base-points ledger event after the trusted notify succeeds.
    ...
    }).catch((error) => ({ ok: false, awarded: false, error: String(error?.message || error || "points_phase1_failed") }));
