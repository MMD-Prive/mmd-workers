...
export async function renderSupplierInvitePage(request, env = {}) {
  ...
  if (!inviteToken) {
    return pageResponse(renderUnavailable(
      "ลิงก์ส่วนตัวนี้ต้องเปิดจากข้อความที่ MMD ส่งให้"
    ), 403);
  }
  ...
  try {
    ...
    return pageResponse(renderUnavailable(
      "ระบบกำลังตรวจลิงก์ส่วนตัว กรุณาลองเปิดใหม่อีกครั้ง"
    ), 503);
  }
...
function renderSupplierInvite(preview, portalUrl) {
  ...
  const contact = preview.contact_name || "Supplier";
  const onDemand = preview.mode === "on_demand";
  const details = onDemand
    ? [
      ["ทำงานแบบสั่งตามออเดอร์", "Himai Shop จะเปิดข้อมูลของ " + scope + " เมื่อมีออเดอร์ที่ต้องดูแล"],
      ["สิ่งที่คุณจะเห็น", "สินค้าและรายการที่เกี่ยวกับ " + scope + " เท่านั้น"],
      ["ลิงก์ส่วนตัว", "ใช้เพื่อยืนยัน LINE ครั้งแรก และจะใช้ซ้ำไม่ได้เมื่อเชื่อมสำเร็จ"],
    ]
    : [
      ["ดูสรุปสินค้า", "ตรวจสถานะสินค้าและสต๊อกของ " + scope + " ได้จาก LINE"],
      ["แจ้งเตือนผ่าน LINE", "เมื่อสต๊อกเข้าเกณฑ์ใกล้หมด ระบบจะส่งแจ้งถึงคุณ"],
      ["ลิงก์ส่วนตัว", "ใช้เพื่อยืนยัน LINE ครั้งแรก และจะใช้ซ้ำไม่ได้เมื่อเชื่อมสำเร็จ"],
    ];
  const steps = [
  ...
  return renderShell([
    "<section class='hero'>",
      ...
      "<p class='lead'>MMD ส่งพื้นที่ Supplier ส่วนตัวสำหรับ <strong>" + escapeHtml(scope) + "</strong> ให้คุณ</p>",
      "<div class='scope'>" + (onDemand ? "สั่งตามออเดอร์" : "Supplier Stock Summary") + "</div>",
    "</section>",
    "<section class='card intro'>",
      "<p class='section-kicker'>MMD ส่งอะไรมา</p>",
      "<h2>" + (onDemand ? "พื้นที่รับงานของคุณใน Himai Shop" : "พื้นที่ดูข้อมูลสินค้าของคุณใน Himai Shop") + "</h2>",
      "<p>" + (onDemand
        ? "ลิงก์นี้พาคุณเข้าไปดูข้อมูลที่เกี่ยวกับออเดอร์ของ " + escapeHtml(scope) + " โดยตรง"
        : "ลิงก์นี้พาคุณเข้าไปดูสรุปสินค้าและสต๊อกของ " + escapeHtml(scope) + " โดยตรง") + "</p>",
    "</section>",
...
body{margin:0;min-height:100vh;background:radial-gradient(circle at 100% 0,rgba(157,141,125,.18),transparent 32%),linear-gradient(180deg,#fbf8f1 0%,var(--bg) 55%,#f2ebdf 100%);color:var(--ink);font:16px/1.58 -apple-system,BlinkMacSystemFont,Noto Sans Thai,IBM Plex Sans Thai,system-ui,sans-serif;-webkit-font-smoothing:antialiased}
...
.brand{font:700 16px/1 Georgia,Times New Roman,serif;letter-spacing:.13em;text-transform:uppercase}
.top span{color:var(--muted);font-size:11px;letter-spacing:.08em;text-transform:uppercase}
.hero{padding:38px 2px 18px}.hero.compact{padding-bottom:10px}
.eyebrow,.section-kicker{margin:0 0 8px;color:var(--taupe);font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase}
h1{margin:0;font:500 clamp(34px,9vw,48px)/1.05 Georgia,Times New Roman,serif;letter-spacing:-.04em}
